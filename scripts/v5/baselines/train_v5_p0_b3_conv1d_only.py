#!/usr/bin/env python3
"""
V5 P0-B3 Conv1D-Only Baseline

Scientific question
-------------------
Does learned temporal order improve over:
- B1: final epoch only;
- B2: order-free temporal mean;

when graph message passing is still forbidden?
...
Architecture
------------
- shared causal depthwise-separable Conv1D encoder per router;
- dilations 1, 2, 4, 8;
- final encoded time step;
- shared node heads and mean/max graph readout;
- no edge_index and no graph message passing.

Protocol
--------
- Same seed, optimizer, epochs, batch size, learning rate, loss weights,
  thresholds, and final-epoch evaluation protocol as B1/B2.
- TRAIN fitting only.
...
def verify_prerequisites(
    ...
    paths = {
        ...
        "b2_report": (
            b2_dir / "V5_P0_B2_TEMPORAL_MEAN_POOLING.json"
        ),
        "b2_lock": (
            b2_dir / "V5_P0_B2_TEMPORAL_MEAN_POOLING_LOCK.json"
        ),
        "b2_marker": (
            b2_dir / "V5_P0_B2_TEMPORAL_MEAN_POOLING_COMPLETE"
        ),
    ...
    b2 = load_json(paths["b2_report"])
    b2_lock = load_json(paths["b2_lock"])
    ...
    if b2.get("status") != "COMPLETE":
        failures.append("B2 status is not COMPLETE")

    integrity_pairs = (
        ("a3", paths["a3_report"], a3_lock),
        ("r1c", paths["r1c_report"], r1c_lock),
        ("b1", paths["b1_report"], b1_lock),
        ("b2", paths["b2_report"], b2_lock),
    )
    for name, report_path, lock in integrity_pairs:
        if lock.get("report_sha256") != sha256_file(report_path):
            failures.append(f"{name} report SHA does not match lock")
    ...
    for stage_name, report in (("B1", b1), ("B2", b2)):
        protocol = report.get("protocol", {})
        for key, expected in expected_protocol.items():
            if protocol.get(key) != expected:
                failures.append(
                    f"{stage_name} protocol mismatch for {key}: "
    ...
    return failures, {
        ...
        "b2_report_sha256": sha256_file(paths["b2_report"]),
        "b2_lock_sha256": sha256_file(paths["b2_lock"]),
...
class Conv1DOnlyModel(nn.Module):
    ...
    def forward(
        ...
        # Causal representation at the target/final epoch.
...
def node_role_metrics(
    ...
    f1 = 2 * precision * recall / max(1e-12, precision + recall)
    exact = float(
        (truth == prediction).all(dim=1).float().mean().item()
    )
...
def evaluate(
    ...
    all_exact = (
        combined["attack_truth"] == combined["attack_prediction"]
    ) & (
        combined["count_truth"] == combined["count_prediction"]
    )
...
def metric_delta(
    ...
    current_key = key_metrics(current)
    reference_key = key_metrics(reference)
    return {
        key: current_key[key] - reference_key[key]
...
def main() -> int:
    ...
    failures, prerequisite_provenance, b1_report, b2_report = (
    ...
    if failures:
        ...
        write_json(
            output_dir / "V5_P0_B3_CONV1D_ONLY.json",
            report,
        )
        atomic_write(
            output_dir / "V5_P0_B3_CONV1D_ONLY_HOLD",
            "V5_P0_B3_CONV1D_ONLY_HOLD\n",
        )
        print("V5_P0_B3_CONV1D_ONLY_HOLD")
        return 1
    ...
    model = Conv1DOnlyModel().to(device)
    ...
    history_path = output_dir / "V5_P0_B3_CONV1D_ONLY_HISTORY.csv"
    metrics_path = output_dir / "V5_P0_B3_CONV1D_ONLY_METRICS.csv"
    ...
    b2_train = b2_report["train_metrics"]
    b2_validation = b2_report["validation_metrics"]

    comparison = {
        "train_b3_minus_b1": metric_delta(train_metrics, b1_train),
        "validation_b3_minus_b1": metric_delta(
            validation_metrics,
            b1_validation,
        ),
        "train_b3_minus_b2": metric_delta(train_metrics, b2_train),
        "validation_b3_minus_b2": metric_delta(
            validation_metrics,
            b2_validation,
        ),
        "key_metrics": {
            "b1_train": key_metrics(b1_train),
            "b1_validation": key_metrics(b1_validation),
            "b2_train": key_metrics(b2_train),
            "b2_validation": key_metrics(b2_validation),
            "b3_train": key_metrics(train_metrics),
            "b3_validation": key_metrics(validation_metrics),
        },
    }

    comparison_path = output_dir / "V5_P0_B3_VS_B1_B2_COMPARISON.json"
    ...
    report = {
        ...
        "status": "COMPLETE",
        "scientific_question": (
            "Does learned temporal order improve over final-epoch-only "
            "and order-free mean pooling without graph message passing?"
        ),
        "model": {
            "class": "Conv1DOnlyModel",
            "parameter_count": parameter_count,
            "node_input": "PRIMARY58 full window + raw physical mask",
            "temporal_encoder": "causal depthwise-separable Conv1D",
            "temporal_dilations": [1, 2, 4, 8],
            "temporal_receptive_field": 31,
            "temporal_order_used": True,
            "edge_index_used": False,
        ...
        "fairness_against_b1_b2": {
            ...
            "same_graph_and_role_readout_dimensions": True,
            "intended_change": (
                "replace static/mean temporal reduction with learned "
                "causal Conv1D while still forbidding graph message passing"
            ),
        ...
        "train_metrics": train_metrics,
        "validation_metrics": validation_metrics,
        "comparison_to_b1_b2": comparison,
        "artifacts": {
        ...
        "next_stage": "V5_P0_B4_GCN_ONLY",
    }

    report_path = output_dir / "V5_P0_B3_CONV1D_ONLY.json"
    write_json(report_path, report)

    lock = {
        "status": "V5_P0_B3_CONV1D_ONLY_COMPLETE",
    ...
    write_json(
        output_dir / "V5_P0_B3_CONV1D_ONLY_LOCK.json",
        lock,
    )

    atomic_write(
        output_dir / "V5_P0_B3_CONV1D_ONLY_COMPLETE",
        "V5_P0_B3_CONV1D_ONLY_COMPLETE\n",
    )

    delta_b2 = comparison["validation_b3_minus_b2"]
    ...
    print("test_split_accessed: false")
    print("next_stage: V5_P0_B4_GCN_ONLY")
    print("V5_P0_B3_CONV1D_ONLY_COMPLETE")
    return 0
