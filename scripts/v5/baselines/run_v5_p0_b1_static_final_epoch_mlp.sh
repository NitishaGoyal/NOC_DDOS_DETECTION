...
SCRIPT="$REPO/scripts/v5/baselines/train_v5_p0_b1_static_final_epoch_mlp.py"
OUT="$REPO/reports/v5/p0_b1_static_final_epoch_mlp"
MODEL_DIR="$REPO/models/v5/p0_b1_static_final_epoch_mlp"
LOG="$REPO/logs/v5/v5_p0_b1_static_final_epoch_mlp.log"

echo "===== V5 P0-B1 STATIC FINAL-EPOCH MLP ====="
echo "repo=$REPO"
echo "data=$DATA"
echo "a2=$A2"
echo "a2_1=$A2_1"
...
for path in "$DATA" "$A2" "$A2_1" "$A3" "$R1C"; do
    if [ ! -d "$path" ]; then
...
echo
echo "v5_p0_b1_status=$status"

if [ -f "$OUT/V5_P0_B1_STATIC_FINAL_EPOCH_MLP_COMPLETE" ]; then
    cat "$OUT/V5_P0_B1_STATIC_FINAL_EPOCH_MLP_COMPLETE"
elif [ -f "$OUT/V5_P0_B1_STATIC_FINAL_EPOCH_MLP_HOLD" ]; then
    cat "$OUT/V5_P0_B1_STATIC_FINAL_EPOCH_MLP_HOLD"
else
