...
SCRIPT="$REPO/scripts/v5/baselines/train_v5_p0_b3_conv1d_only.py"
OUT="$REPO/reports/v5/p0_b3_conv1d_only"
MODEL_DIR="$REPO/models/v5/p0_b3_conv1d_only"
LOG="$REPO/logs/v5/v5_p0_b3_conv1d_only.log"

echo "===== V5 P0-B3 CONV1D-ONLY ====="
echo "repo=$REPO"
echo "data=$DATA"
echo "a2=$A2"
echo "a2_1=$A2_1"
...
for path in "$DATA" "$A2" "$A2_1" "$A3" "$R1C" "$B1" "$B2"; do
    if [ ! -d "$path" ]; then
...
echo
echo "v5_p0_b3_status=$status"

if [ -f "$OUT/V5_P0_B3_CONV1D_ONLY_COMPLETE" ]; then
    cat "$OUT/V5_P0_B3_CONV1D_ONLY_COMPLETE"
elif [ -f "$OUT/V5_P0_B3_CONV1D_ONLY_HOLD" ]; then
    cat "$OUT/V5_P0_B3_CONV1D_ONLY_HOLD"
else
