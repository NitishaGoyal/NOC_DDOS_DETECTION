...
SCRIPT="$REPO/scripts/v5/baselines/train_v5_p0_b4_gcn_only.py"
OUT="$REPO/reports/v5/p0_b4_gcn_only"
MODEL_DIR="$REPO/models/v5/p0_b4_gcn_only"
LOG="$REPO/logs/v5/v5_p0_b4_gcn_only.log"

echo "===== V5 P0-B4 GCN-ONLY ====="
echo "repo=$REPO"
echo "data=$DATA"
echo "a2=$A2"
echo "a2_1=$A2_1"
...
for path in "$DATA" "$A2" "$A2_1" "$A3" "$R1C" "$B1" "$B2" "$B3"; do
    if [ ! -d "$path" ]; then
...
echo
echo "v5_p0_b4_status=$status"
if [ -f "$OUT/V5_P0_B4_GCN_ONLY_COMPLETE" ]; then
    cat "$OUT/V5_P0_B4_GCN_ONLY_COMPLETE"
elif [ -f "$OUT/V5_P0_B4_GCN_ONLY_HOLD" ]; then
    cat "$OUT/V5_P0_B4_GCN_ONLY_HOLD"
else
