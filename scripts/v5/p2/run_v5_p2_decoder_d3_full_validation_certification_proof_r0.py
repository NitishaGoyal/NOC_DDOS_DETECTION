...
def main() -> None:
    ...
    # R0 permits only the exact partial-finalization state produced by the
    # original slots-dataclass serialization bug. A completed or materially
    # different final output remains immutable and causes a hard failure.
    if (
        final_artifact_json.exists()
        or final_report_json.exists()
        or final_complete_marker.exists()
    ):
        raise RuntimeError(
            "completed/final D3 output already exists; do not overwrite it"
        )

    if final_report_dir.exists():
        unexpected_report_entries = sorted(
            path.name for path in final_report_dir.iterdir()
        )
        if unexpected_report_entries:
            raise RuntimeError(
                "partial D3 report directory contains unexpected entries: "
                f"{unexpected_report_entries}"
            )

    if final_artifact_dir.exists():
        unexpected_artifact_entries = sorted(
            path.name
            for path in final_artifact_dir.iterdir()
            if path.name != "certified_outputs"
        )
        if unexpected_artifact_entries:
            raise RuntimeError(
                "partial D3 artifact directory contains unexpected entries: "
                f"{unexpected_artifact_entries}"
            )
        existing_outputs = final_artifact_dir / "certified_outputs"
        if existing_outputs.exists() and not existing_outputs.is_dir():
            raise RuntimeError(
                "partial certified_outputs entry is not a directory"
            )
    ...
    # The original D3 runner created these directories and hard links before
    # failing while serializing a slots dataclass. Reuse only hash-identical
    # hard links; never overwrite a pre-existing output.
    unexpected_report_entries = sorted(
        path.name for path in final_report_dir.iterdir()
    )
    if unexpected_report_entries:
        raise RuntimeError(
            "D3 report directory is not the known empty partial state: "
            f"{unexpected_report_entries}"
        )
    ...
    if unexpected_artifact_entries:
        raise RuntimeError(
            "D3 artifact directory is not the known partial state: "
            f"{unexpected_artifact_entries}"
        )

    certified_outputs_dir = final_artifact_dir / "certified_outputs"
    certified_outputs_dir.mkdir(exist_ok=True)

    expected_output_names = {
        str(record["filename"]) for record in records
    }
    unexpected_output_names = sorted(
        path.name
        for path in certified_outputs_dir.iterdir()
        if path.name not in expected_output_names
    )
    if unexpected_output_names:
        raise RuntimeError(
            "partial certified-output archive contains unexpected files: "
            f"{unexpected_output_names}"
        )

    reused_partial_hardlinks = 0
    newly_linked_outputs = 0
    for record in records:
        ...
        if destination.exists():
            if not destination.is_file():
                raise RuntimeError(
                    f"existing certified output is not a file: {destination}"
                )
            if sha256_file(destination) != str(record["sha256"]):
                raise RuntimeError(
                    f"existing certified output hash mismatch: {destination}"
                )
            if not os.path.samefile(source, destination):
                raise RuntimeError(
                    "existing certified output is not the expected hard link: "
                    f"{destination}"
                )
            reused_partial_hardlinks += 1
        else:
            os.link(source, destination)
            newly_linked_outputs += 1
    ...
    proof = {
        ...
        "solver_identity": asdict(
            certified.solver_identity()
        ),
        "R0_finalization_patch": {
            "classification": (
                "POST_CERTIFICATION_REPORT_SERIALIZATION_ABORT"
            ),
            "cause": (
                "SolverIdentity is a frozen slots dataclass and therefore "
                "does not expose __dict__"
            ),
            "scientific_results_changed": False,
            "working_chunks_redecoded": False,
            "working_chunks_hash_verified": True,
            "partial_hardlinks_reused": reused_partial_hardlinks,
            "new_hardlinks_created": newly_linked_outputs,
            "determinism_replay_repeated": True,
        },
...
""",
    ...
    print("D3_R0_slots_dataclass_finalization_patch=true")
    print(
        f"partial_hardlinks_reused={reused_partial_hardlinks}"
    )
    print(
        f"new_hardlinks_created={newly_linked_outputs}"
    )
