...
TRAINER="$REPO/scripts/v5/p2/train_v5_p2_g2_direct_graph_single_run.py"
SCRIPT="$REPO/scripts/v5/p2/preflight_v5_p2_g2_clean_training_implementation.py"
OUT="$REPO/reports/v5/p2_g2_clean_training_implementation_preflight"
LOG="$REPO/logs/v5/v5_p2_g2_clean_training_implementation_preflight.log"
COMPLETE="$OUT/V5_P2_G2_CLEAN_TRAINING_IMPLEMENTATION_PREFLIGHT_COMPLETE"
HOLD="$OUT/V5_P2_G2_CLEAN_TRAINING_IMPLEMENTATION_PREFLIGHT_HOLD"

if [ -f "$COMPLETE" ]; then
    echo "G2 clean preflight already complete"
    cat "$COMPLETE"
    exit 0
fi
for path in "$DATA" "$G0" "$TOPOLOGY" "$B0_R3" "$G1_FREEZE" "$G1_AGG"; do
...
if [ -e "$OUT" ] || [ -e "$LOG" ]; then
    echo "STOP: partial or pre-existing G2 preflight output exists"
    echo "output_dir=$OUT"
    echo "log=$LOG"
    [ -f "$HOLD" ] && cat "$HOLD"
    exit 2
...
python -u "$SCRIPT" \
    ...
    2>&1 | tee "$LOG"
status=${PIPESTATUS[0]}
echo "v5_p2_g2_clean_preflight_status=$status"
