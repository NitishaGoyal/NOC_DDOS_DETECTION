...
def main() -> None:
    ...
    if f1.get("recovery_authorization_consumed") is not True:
        raise RuntimeError("F1 recovery authorization state changed")
    if f1.get("recovery_rerun_authorized") is not False:
        raise RuntimeError("F1 unexpectedly authorizes a rerun")
    if f1.get("official_complete_test_result_obtained") is not False:
