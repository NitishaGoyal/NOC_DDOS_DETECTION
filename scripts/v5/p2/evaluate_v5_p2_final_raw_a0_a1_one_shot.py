#!/usr/bin/env python3
"""Immutable V5 P2 Raw/A0/A1 one-shot blind-test evaluator.

This source is frozen before authorization. It contains no training, checkpoint
...
6. Only then resolve/check/enumerate ``data_root/runs/test``.
...
STAGE = "V5_P2_FINAL_RAW_A0_A1_ONE_SHOT_BLIND_EVALUATION"
...
def _manifest_int(row: dict[str, str], key: str) -> int:
    try:
        return int(row[key])
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(
            f"invalid manifest integer {key!r} for {row.get('pair_key')!r}"
        ) from exc
...
class FrozenP2TestDataset(Dataset):
    """Pair-aligned frozen P2 test loader; instantiate only after authorization."""
    ...
    def __init__(
        ...
        *,
        ...
        for manifest_row_index, row in enumerate(all_rows):
            if row.get("split") != "test":
                continue
            pair_key = row.get("pair_key", "")
            if not pair_key:
                raise ValueError("test manifest row has empty pair_key")
            if pair_key in seen_pairs:
                raise ValueError(f"duplicate test pair_key: {pair_key}")
            seen_pairs.add(pair_key)

            pair_id = row.get("pair_id", "")
            category = row.get("category", "")
            common_length = _manifest_int(row, "common_length")
            window_count = _manifest_int(row, "window_count_per_member")
            manifest_window = _manifest_int(row, "window")
            manifest_stride = _manifest_int(row, "stride")
            if manifest_window != WINDOW or manifest_stride != STRIDE:
                raise ValueError(
                    f"{pair_key}: window/stride changed "
                    f"({manifest_window},{manifest_stride})"
                )

            expected_window_count = (
                0
                if common_length < WINDOW
                else 1 + (common_length - WINDOW) // STRIDE
            )
            if window_count != expected_window_count:
                raise ValueError(
                    f"{pair_key}: window count={window_count}, "
                    f"expected={expected_window_count}"
                )
            if window_count <= 0:
                raise ValueError(f"{pair_key}: no aligned test windows")

            attack_name = f"{pair_key}_ATTACK.pt"
            control_name = f"{pair_key}_CONTROL.pt"
            expected_names.update((attack_name, control_name))
            attack_path = test_dir / attack_name
            control_path = test_dir / control_name
            ...
            pair_record = {
                ...
                "aligned_item_count": 2 * window_count,
                "window": WINDOW,
                "stride": STRIDE,
                "attack_file_name": attack_name,
                "control_file_name": control_name,
            }
            self._pair_rows.append(pair_record)
        ...
        if not self._index or len(self._index) % 2 != 0:
            raise RuntimeError("invalid pair-aligned test index")
        ...
        if actual_pt_names != expected_names:
            missing = sorted(expected_names - actual_pt_names)
            unexpected = sorted(actual_pt_names - expected_names)
            raise RuntimeError(
                f"test tensor set differs from frozen manifest: "
    ...
    def stable_metadata(self, index: int) -> dict[str, Any]:
        ...
        return {
            ...
            "manifest_row_index": entry.manifest_row_index,
            "stable_item_id": (
                f"test/{entry.pair_key}/{entry.pair_id}/{entry.mode}/"
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        if set(item) != REQUIRED_ITEM_KEYS:
            raise RuntimeError("test loader item-key contract changed")
        return item
...
def validate_authorization(
    *,
    ...
    if contract.get("stage") != "V5_P2_FINAL_ONE_SHOT_AUTHORIZATION":
        raise RuntimeError("authorization contract stage mismatch")
    ...
    *,
    ...
    try:
        ...
        report = {
            "stage": STAGE,
            "status": "COMPLETE",
            "decision": "FINAL_P2_RAW_A0_A1_BLIND_TEST_RESULT_RECORDED",
            "authorization": {
                "authorization_id": token["authorization_id"],
        ...
        print(
            "Authorization was consumed. Do not rerun or delete guard artifacts."
        )
        return 1
...
def main() -> int:
    ...
    if args.source_only_self_test:
        ...
        print("V5_P2_FINAL_EVALUATOR_SOURCE_ONLY_SELF_TEST_PASS")
