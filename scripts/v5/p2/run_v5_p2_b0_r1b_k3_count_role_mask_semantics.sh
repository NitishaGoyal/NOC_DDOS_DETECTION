...
B0_R1A="$REPO/reports/v5/p2_b0_r1a_k3_count_role_mask_semantics_audit"

MODEL="$REPO/src/models/v5_frozen_b3_conv1d_only.py"
SCRIPT="$REPO/scripts/v5/p2/audit_v5_p2_b0_r1b_k3_count_role_mask_semantics.py"

OUT="$REPO/reports/v5/p2_b0_r1b_k3_count_role_mask_semantics_audit"
LOG="$REPO/logs/v5/v5_p2_b0_r1b_k3_count_role_mask_semantics_audit.log"

echo "===== V5 P2-B0-R1B K3 COUNT + ROLE-MASK SEMANTICS ====="
echo "repo=$REPO"
echo "data=$DATA"
echo "a1_r2=$A1_R2"
echo "a3=$A3"
...
echo "historical_b0_r1a_hold=$B0_R1A"
echo "model=$MODEL"
echo "script=$SCRIPT"
echo "output=$OUT"
echo "log=$LOG"
echo "historical_security_schema=nested_or_explicit_top_level_false"
...
echo
echo "v5_p2_b0_r1b_status=$status"

if [ -f "$OUT/V5_P2_B0_R1B_K3_COUNT_AND_ROLE_MASK_SEMANTICS_AUDIT_COMPLETE" ]; then
    cat "$OUT/V5_P2_B0_R1B_K3_COUNT_AND_ROLE_MASK_SEMANTICS_AUDIT_COMPLETE"
elif [ -f "$OUT/V5_P2_B0_R1B_K3_COUNT_AND_ROLE_MASK_SEMANTICS_AUDIT_HOLD" ]; then
    cat "$OUT/V5_P2_B0_R1B_K3_COUNT_AND_ROLE_MASK_SEMANTICS_AUDIT_HOLD"
else
