...
case "$OPERATOR" in
    ...
    *) echo "Usage: $0 {conv1d|gcnconv|graphconv} {107|117|127}"; exit 2 ;;
esac
case "$SEED" in
    107|117|127) ;;
    *) echo "Usage: $0 {conv1d|gcnconv|graphconv} {107|117|127}"; exit 2 ;;
...
PREFLIGHT="$REPO/reports/v5/p2_g3_clean_training_implementation_preflight"
...
SCRIPT="$REPO/scripts/v5/p2/train_v5_p2_g3_role_aware_multilabel_single_run.py"

MODEL_DIR="$REPO/models/v5/p2_g3_role_aware_multilabel/$OPERATOR/seed_${SEED}"
REPORT_DIR="$REPO/reports/v5/p2_g3_role_aware_multilabel_training/$OPERATOR/seed_${SEED}"
LOG="$REPO/logs/v5/v5_p2_g3_${OPERATOR}_seed_${SEED}.log"
COMPLETE_MARKER="$REPORT_DIR/V5_P2_G3_ROLE_AWARE_MULTILABEL_SINGLE_RUN_COMPLETE"
HOLD_MARKER="$REPORT_DIR/V5_P2_G3_ROLE_AWARE_MULTILABEL_SINGLE_RUN_HOLD"
...
for path in "$DATA" "$G0" "$TOPOLOGY" "$B0_R3" "$G1_AGG" "$G2_AGG" "$PREFLIGHT"; do
...
echo "===== V5 P2-G3 ROLE-AWARE MULTILABEL RUN ====="
echo "operator=$OPERATOR"
echo "seed=$SEED"
echo "execution=serial_one_model_at_a_time"
echo "automatic_mixed_precision=false"
echo "roles=source,transit,victim,path"
echo "roles_mutually_exclusive=false"
...
status=${PIPESTATUS[0]}
echo
echo "v5_p2_g3_${OPERATOR}_seed_${SEED}_status=$status"
if [ -f "$COMPLETE_MARKER" ]; then
    cat "$COMPLETE_MARKER"
