...
SCRIPT="$REPO/scripts/v5/p2/audit_v5_p2_b0_r1_k3_count_semantics_head_compatibility.py"

OUT="$REPO/reports/v5/p2_b0_r1_k3_count_semantics_head_compatibility"
LOG="$REPO/logs/v5/v5_p2_b0_r1_k3_count_semantics_head_compatibility.log"

echo "===== V5 P2-B0-R1 K3 COUNT SEMANTICS + HEAD COMPATIBILITY ====="
echo "repo=$REPO"
echo "data=$DATA"
echo "a1_r2=$A1_R2"
echo "a3=$A3"
...
for path in "$DATA" "$A1_R2" "$A3" "$A4" "$B0"; do
    [ -d "$path" ] || {
...
echo
echo "v5_p2_b0_r1_status=$status"

if [ -f "$OUT/V5_P2_B0_R1_K3_COUNT_SEMANTICS_AND_HEAD_COMPATIBILITY_AUDIT_COMPLETE" ]; then
    cat "$OUT/V5_P2_B0_R1_K3_COUNT_SEMANTICS_AND_HEAD_COMPATIBILITY_AUDIT_COMPLETE"
elif [ -f "$OUT/V5_P2_B0_R1_K3_COUNT_SEMANTICS_AND_HEAD_COMPATIBILITY_AUDIT_HOLD" ]; then
    cat "$OUT/V5_P2_B0_R1_K3_COUNT_SEMANTICS_AND_HEAD_COMPATIBILITY_AUDIT_HOLD"
else
