...
case "$OPERATOR" in
    ...
    *) echo "Usage: $0 {conv1d|gcnconv|graphconv|gatconv} {107|117|127}"; exit 2 ;;
esac
case "$SEED" in
    107|117|127) ;;
    *) echo "Usage: $0 {conv1d|gcnconv|graphconv|gatconv} {107|117|127}"; exit 2 ;;
...
PREFLIGHT="$REPO/reports/v5/p2_g2_clean_training_implementation_preflight"
...
SCRIPT="$REPO/scripts/v5/p2/train_v5_p2_g2_direct_graph_single_run.py"

MODEL_DIR="$REPO/models/v5/p2_g2_direct_graph/$OPERATOR/seed_${SEED}"
REPORT_DIR="$REPO/reports/v5/p2_g2_direct_graph_training/$OPERATOR/seed_${SEED}"
LOG="$REPO/logs/v5/v5_p2_g2_${OPERATOR}_seed_${SEED}.log"
COMPLETE_MARKER="$REPORT_DIR/V5_P2_G2_DIRECT_GRAPH_SINGLE_RUN_COMPLETE"
HOLD_MARKER="$REPORT_DIR/V5_P2_G2_DIRECT_GRAPH_SINGLE_RUN_HOLD"
...
for path in "$DATA" "$G0" "$TOPOLOGY" "$B0_R3" "$G1_FREEZE" "$G1_AGG" "$PREFLIGHT"; do
...
echo "===== V5 P2-G2 DIRECT GRAPH RUN ====="
...
status=${PIPESTATUS[0]}
echo
echo "v5_p2_g2_${OPERATOR}_seed_${SEED}_status=$status"
if [ -f "$COMPLETE_MARKER" ]; then
    cat "$COMPLETE_MARKER"
