...
STAGE = "V5_P3_F7_E2_R6_R1_R1_TOP_LEVEL_FUNCTION_RESOLUTION_RECOVERY"
...
def create_recursive_batch_device_transfer_trainer(
    ...
    # Protect all scientific functions from accidental modification.
    original_scientific_sources = {
        name: exact_function_source(text, tree, name)
        for name in ("compute_loss", "train_one_epoch", "evaluate")
    }
    corrected_scientific_sources = {
        name: exact_function_source(corrected, corrected_tree, name)
        for name in ("compute_loss", "train_one_epoch", "evaluate")
    }
    require(
        original_scientific_sources == corrected_scientific_sources,
        "scientific train/loss/evaluation source changed during move_batch patch",
    )
    ...
    return {
        ...
        "function_source_resolution": "module_level_tree_body_only",
        "nested_same_name_functions_ignored_for_scientific_source_guard": True,
        "compute_loss_unchanged": True,
        "train_one_epoch_unchanged": True,
...
def main() -> int:
    ...
    resolver_self_test_tree = ast.parse(resolver_self_test_text)
    resolver_self_test_source = exact_function_source(
        resolver_self_test_text,
        resolver_self_test_tree,
        "evaluate",
    )
    require(
        "return 'module'" in resolver_self_test_source,
        "module-level function resolver self-test failed",
    )
    ...
    legacy_guard_corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r6_r1_legacy_guards.py"
    )
    ...
    item_batch_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r6_r1_item_batch.py"
    )
    ...
    corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r6_r1.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r6_r1.sh"
    )
    require(
        "shlex" in globals(),
    ...
    scratch = output_dir / "F7_E2_R6_R1_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "R6_function_source_guard_used_ast_walk_and_confused_top_level_evaluate_with_nested_evaluate_helpers"
        ),
    ...
    launcher_patch_path = output_dir / (
        "F7_E2_R6_R1_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R6_R1_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R6_R1_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R6_R1_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    decision = {
        ...
        "module_level_function_resolution_verified": True,
        "nested_evaluate_name_collision_ignored": True,
    ...
    print("function_source_resolution=module_level_tree_body_only")
    print("nested_evaluate_name_collision_ignored=true")
    print("module_level_function_resolution_self_test=true")
