...
STAGE = "V5_P3_F7_E2_R2_NONEXISTENT_OUTPUT_DIR_AND_LAUNCHER_RECOVERY"
...
def main() -> int:
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r2.sh"
    )
    ...
    scratch = output_dir / "F7_E2_R2_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    launcher_patch_path = output_dir / (
        "F7_E2_R2_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R2_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R2_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R2_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    print(f"generated_trainer={trainer_path}")
