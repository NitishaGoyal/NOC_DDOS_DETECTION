...
STAGE = "V5_P3_F7_E1_R3_REQUIRED_LEGACY_CLI_BINDING"
CAMPAIGN = "V5-P3-F0-D70-FEATURE-STUDY"
CLASSIFICATION = "TRAIN-DERIVED"
FAILURE_CLASSIFICATION = (
    "E1_R3_data_cli_not_present_dataset_route_is_environment_injected"
)
...
RUNTIME_ADAPTER_RELATIVE_PATH = Path(
    "src/models/v5_p3_f7_reconstructed_runtime_adapter_e1_r3.py"
)
...
def _build_fresh_model() -> torch.nn.Module:
    ...
    f6 = _import_source(
        f6_source,
        "_v5_p3_f7_e1_r3_runtime_model_capture",
    )
...
class _F7FixedSplitDatasetAdapter(Dataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loader = _import_source(
            loader_path,
            f"_v5_p3_f7_e1_r3_loader_{self.FIXED_SPLIT}",
        )
...
def cli_row_by_dest(
    ...
    matches = [
        dict(row)
        for row in rows
        if normalize(row["dest"]) == normalize(dest)
    ]
    require(
        len(matches) == 1,
        f"required CLI destination {dest!r} is not unique: "
        f"{[row['dest'] for row in matches]}",
    )
    row = matches[0]
    long_options = [
        option
    ...
    require(
        long_options or row["options"],
        f"required CLI destination {dest!r} has no option string",
    )
    row["selected_option"] = (
        long_options[-1] if long_options else row["options"][0]
    )
...
def _lineage_text(directories: list[Path]) -> str:
    suffixes = {
...
def _unique_best_directory(
    repo: Path,
    token_groups: tuple[str, ...],
) -> tuple[Path, list[dict[str, Any]]]:
    root = repo / "reports/v5"
    require(root.is_dir(), f"reports/v5 missing: {root}")
    ...
    for path in root.rglob("*"):
        ...
        token = normalize(relative)
        if not all(normalize(part) in token for part in token_groups):
            continue

        basename = normalize(path.name)
        score = 0
        if all(normalize(part) in basename for part in token_groups):
            score += 100
        if basename.startswith("_".join(normalize(part) for part in token_groups)):
            score += 100
        file_count = sum(1 for child in path.rglob("*") if child.is_file())
        json_count = sum(
            1 for child in path.rglob("*.json")
            if child.is_file()
        )
        score += min(file_count, 30)
        score += min(json_count * 2, 30)
        candidates.append({
    ...
    require(
        candidates,
        f"no directory candidate found for tokens={token_groups}",
    )
    candidates.sort(
        key=lambda row: (-row["score"], row["relative"])
    )
    ...
    require(
        len(best) == 1,
        f"directory resolution ambiguous for tokens={token_groups}: "
        f"{best}",
    )
    return Path(best[0]["path"]), candidates


def _extract_python_path_mentions(
    text: str,
    repo: Path,
) -> dict[Path, int]:
    pattern = re.compile(
    ...
    for match in pattern.findall(text):
        ...
        if raw_path.is_absolute():
            ...
            candidates.append(repo / raw.lstrip("./"))
...
def _resolve_source_path(
    ...
    *,
    ...
    require(kind in {"loader", "model"}, f"unknown source kind: {kind}")
    mentions = _extract_python_path_mentions(
        lineage_text,
        repo,
    )

    search_root = repo / ("src/data" if kind == "loader" else "src/models")
    if search_root.is_dir():
        for path in search_root.rglob("*.py"):
            mentions.setdefault(path.resolve(), 0)

    rows = []
    for path, mention_count in mentions.items():
        relative = (
        ...
        token = normalize(relative)
        score = mention_count * 100

        if kind == "loader":
            if "src_data" in token:
                score += 70
            if "loader" in token:
                score += 60
            if "dataset" in token:
        ...
        if "v5_p3" in token:
            score -= 25
        if "experiment" in token or "report" in token:
            score -= 20
    ...
    require(rows, f"no {kind} source candidates found")
    rows.sort(key=lambda row: (-row["score"], row["relative"]))
    best_score = rows[0]["score"]
    best = [row for row in rows if row["score"] == best_score]
    require(
        len(best) == 1,
        f"{kind} source resolution ambiguous: {best[:10]}",
    )
    require(
        best[0]["mention_count"] > 0,
        f"best {kind} source is not cited by the frozen B1/B0-R3 lineage: "
        f"{best[0]}",
    )
    return Path(best[0]["path"]), rows


def resolve_legacy_cli_bindings(
    repo: Path,
    cli_rows: list[dict[str, Any]],
) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    b1_dir = (
        repo / "reports/v5/p2_b1_training_protocol_lock"
    ).resolve()
    require(
        b1_dir.is_dir(),
        f"frozen P2 B1 directory missing: {b1_dir}",
    )

    b0_r3_dir, b0_candidates = _unique_best_directory(
        repo,
        ("p2", "b0", "r3"),
    )

    lineage = _lineage_text([b1_dir, b0_r3_dir])
    require(
        lineage,
        "B1/B0-R3 lineage text is empty",
    )

    loader_path, loader_candidates = _resolve_source_path(
        repo,
        lineage,
        kind="loader",
    )
    model_path, model_candidates = _resolve_source_path(
        repo,
        lineage,
        kind="model",
    )
    ...
    for dest, value in values.items():
        row = cli_row_by_dest(cli_rows, dest)
        bindings[dest] = {
            "dest": dest,
    ...
    evidence = {
        ...
        "b0_r3_candidates": b0_candidates,
        "loader_candidates": loader_candidates,
        "model_candidates": model_candidates,
        "lineage_text_sha256": sha256_text(lineage),
        "lineage_character_count": len(lineage),
    }
...
def matrix_launcher_source(
    ...
    for dest in (
        ...
        binding = legacy_bindings[dest]
        legacy_fragments.append(
            "    "
            + shlex.quote(binding["selected_option"])
            + " "
            + shlex.quote(binding["value"])
            + "\\n"
        )
    legacy_fragment = "".join(legacy_fragments)

    data_fragment = ""
    if cli_map.get("data") is not None:
        data_fragment = (
            "    "
            + shlex.quote(cli_map["data"]["selected_option"])
            + ' "$DATA_LINK"\\n'
        )
...
for SPEC in "$MATRIX_DIR"/*.json; do
  ...
  CMD=(
    ...
    "$TRAINER"
    {seed_option} "107"
{legacy_fragment}{data_fragment}    {output_option} "$RUN_DIR"
  )
{device_fragment}
  printf 'launching %s\\n' "$LABEL"
  printf 'command:'
...
def main() -> int:
    ...
    import_text = (
        "from src.models.v5_p3_f7_reconstructed_runtime_adapter_e1_r3 "
        "import (\n"
    ...
    generated_text = (
        "# AUTO-GENERATED BY V5-P3 F7-E1-R3.\n"
    ...
    legacy_bindings, legacy_binding_evidence = (
        resolve_legacy_cli_bindings(
            repo,
            cli_rows,
        )
    )
    ...
    selected_dests.update(legacy_bindings)
    ...
    matrix_dir = f7_root / "F7_E1_R3_PRIMARY_SEED107_RUN_SPECS"
    ...
    route_inventory_path = output_dir / (
        "F7_E1_R3_CONSTRUCTOR_ASSIGNMENT_ROUTE_INVENTORY.json"
    )
    patch_map_path = output_dir / (
        "F7_E1_R3_GENERATED_SOURCE_AND_CALLSITE_PATCH_MAP.json"
    )
    matrix_manifest_path = output_dir / (
        "F7_E1_R3_PRIMARY_SEED107_MATRIX_MANIFEST.json"
    )
    cli_map_path = output_dir / (
        "F7_E1_R3_GENERATED_TRAINER_CLI_BINDING_MAP.json"
    )
    authorization_path = output_dir / (
        "F7_E1_R3_E2_PREFLIGHT_AUTHORIZATION.json"
    )
    ...
    atomic_json(
        cli_map_path,
        {
            ...
            "legacy_required_bindings": legacy_bindings,
            "legacy_binding_evidence": legacy_binding_evidence,
    ...
    authorization = {
        ...
        "legacy_required_provenance_CLI_bound": True,
        "legacy_binding_values": {
            key: value["value"]
            for key, value in legacy_bindings.items()
        },
        "model_dir_is_per_run_output_directory": True,
        "legacy_data_cli_present": cli_map["data"] is not None,
        "dataset_runtime_route": (
            "F7_DATA_LINK_environment_plus_fixed_guarded_split_adapters"
        ),
    ...
    for dest in (
        ...
        binding = legacy_bindings[dest]
        print(
            "legacy_required_binding="
            f"{dest}:option={binding['selected_option']}:"
            f"value={binding['value']}"
        )
    print(
        "model_dir_output_binding="
        f"option={cli_map['output']['selected_option']}:"
        "value=<per-run RUN_DIR>"
    )
    print(
        "cli_seed_option="
    ...
    print(
        "cli_data_option="
        f"{cli_map['data']['selected_option'] if cli_map['data'] else None}"
    )
    print(
        "dataset_runtime_route="
        "F7_DATA_LINK_environment_plus_fixed_guarded_split_adapters"
