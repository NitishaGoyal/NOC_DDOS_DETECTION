...
STAGE = "V5_P3_F7_E2_R4_R2_R1_FROZEN_BATCH_GEOMETRY_RECOVERY"
...
def main() -> int:
    ...
    cardinality_corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4_r2_cardinality.py"
    )
    ...
    corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4_r2.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r4_r2.sh"
    )
    require(
        "shlex" in globals(),
    ...
    scratch = output_dir / "F7_E2_R4_R2_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "R4_R1_rejected_preserved_train_sampler_set_epoch_use_after_pair_sampler_removal"
        ),
    ...
    cardinality_patch_path = output_dir / (
        "F7_E2_R4_R2_DATASET_CARDINALITY_GUARD_PATCH.json"
    )
    atomic_json(cardinality_patch_path, cardinality_patch)

    item_batch_patch_path = output_dir / (
        "F7_E2_R4_R2_PAIRBLOCK_TO_ITEMBATCH_ROUTE_PATCH.json"
    )
    atomic_json(item_batch_patch_path, item_batch_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R4_R2_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R4_R2_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R4_R2_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R4_R2_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
