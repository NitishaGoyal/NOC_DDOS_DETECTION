...
STAGE = "V5_P3_F7_E1_R4_LEGACY_PATH_ARGUMENT_USE_SITE_AUDIT"
...
def main() -> int:
    ...
    summary_path = output_dir / (
        "F7_E1_R4_LEGACY_PATH_ARGUMENT_USE_SITE_SUMMARY.json"
    )
    full_audit_path = output_dir / (
        "F7_E1_R4_LEGACY_PATH_ARGUMENT_FULL_STATIC_AUDIT.json"
    )
    excerpts_path = output_dir / (
        "F7_E1_R4_LEGACY_PATH_ARGUMENT_EXACT_SOURCE_EXCERPTS.txt"
    )
    decision_path = output_dir / (
        "F7_E1_R4_NEXT_BINDING_ROUTE_DECISION.json"
    )
    ...
    report = {
        ...
        "classification": CLASSIFICATION,
        "scope": (
            "Statically audit every definition, alias, call argument, "
            "comparison, manifest/log insertion, file read, hash, copy, "
            "dynamic import, execution, output and path-normalization use "
            "of root, b1_dir, b0_r3_dir, loader_path, model_path and "
            "model_dir in the frozen V5-P2 skeleton. No pointed-to path is "
            "opened or executed. Use the exact use sites to select the next "
            "compatibility route instead of guessing legacy source paths."
        ),
