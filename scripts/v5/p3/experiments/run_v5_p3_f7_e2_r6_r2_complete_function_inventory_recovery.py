...
STAGE = "V5_P3_F7_E2_R6_R2_R1_TOP_LEVEL_FUNCTION_RESOLUTION_RECOVERY"
...
def create_corrected_legacy_guard_trainer(
    original_path: Path,
    corrected_path: Path,
    expected_original_sha256: str,
) -> dict[str, Any]:
    """Patch legacy P2 cardinality and model-size guards by parsed value.
    ...
    nodes whose evaluated values are exactly 70166, 12528, or 43273.

    The 43273 value is an inherited P2-model guard. The runtime adapter and
    two fresh seed-107 model constructions already certify that the P3
    Dynamic70 model has exactly 60553 trainable parameters.
    """
    require(
    ...
    for old_value, new_value in replacements_by_value.items():
        ...
        require(
            nodes,
            f"legacy compatibility-guard AST integer {old_value} not found",
        )
        require(
            len(nodes) <= 20,
            f"unexpectedly many AST occurrences of legacy guard "
    ...
    require(
        ...
        f"legacy compatibility-guard AST constants survived: {remaining_old_values}",
    )
    ...
    require(
        60553 in corrected_values,
        "certified Dynamic70 parameter count missing after AST correction",
    )

    parameter_count_assignments = []
    for node in ast.walk(corrected_tree):
        ...
        for target in targets:
            if "PARAMETER_COUNT" in target.upper():
                parameter_count_assignments.append({
    ...
    require(
        parameter_count_assignments,
        "no parameter-count constant assignment found after correction",
    )
    require(
        any(
            row["value"] == 60553
            for row in parameter_count_assignments
        ),
        f"no parameter-count assignment resolves to 60553: "
        f"{parameter_count_assignments}",
    )
    require(
        all(
            row["value"] != 43273
        ...
        f"legacy parameter count survived in assignment: "
        f"{parameter_count_assignments}",
    )

    parameter_count_comparisons = [
        {
            "line": int(node.lineno),
            "source": ast.unparse(node),
        }
        ...
        and "parameter_count" in ast.unparse(node).lower()
    ]
    require(
        parameter_count_comparisons,
        "no parameter_count comparison found after legacy-guard correction",
    )
    ...
    return {
        ...
        "parameter_count_assignments": parameter_count_assignments,
        "parameter_count_comparisons": parameter_count_comparisons,
        "failure_classification": (
            "instrumented_trainer_retained_legacy_P2_parameter_count_"
            "legacy_move_batch_called_to_on_list_metadata"
        ),
        "legacy_train_items": 70166,
        "certified_F7_train_items": 110855,
        "legacy_validation_items": 12528,
        "certified_F7_validation_items": 13863,
        "legacy_P2_parameter_count": 43273,
        "certified_Dynamic70_parameter_count": 60553,
        "AST_integer_replacement_counts": counts,
        "exact_source_contexts": contexts,
        "patch_scope": (
            "ast.Constant integer nodes only; evaluated_value "
            "70166_to_110855, 12528_to_13863, and "
            "43273_to_60553"
        ),
...
def create_recursive_batch_device_transfer_trainer(
    item_batch_trainer_path: Path,
    final_path: Path,
    expected_item_batch_sha256: str,
) -> dict[str, Any]:
    """Make device transfer type-aware without changing batch contents.

    The P3 runtime adapter includes non-tensor provenance/metadata leaves.
    PyTorch default collation correctly keeps string metadata as Python lists.
    The inherited P2 ``move_batch`` function called ``.to(...)`` on every
    top-level value, which is valid only when every value is a tensor.

    This patch preserves the exact tensor ``.to(...)`` call from the trainer,
    but applies it recursively only to torch.Tensor leaves. Dict/list/tuple
    structure and all non-tensor leaves are preserved exactly.
    """
    require(
        sha256_file(item_batch_trainer_path)
        == expected_item_batch_sha256,
        "item-batch trainer SHA changed before device-transfer correction",
    )

    text = item_batch_trainer_path.read_text(
        encoding="utf-8",
        errors="replace",
    )
    tree = ast.parse(text)
    ...
    functions = [
        ...
        for node in tree.body
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))
        and node.name == "move_batch"
    ]
    require(
        len(functions) == 1,
        f"move_batch function is not unique: "
        f"{[(node.lineno, ast.unparse(node)) for node in functions]}",
    )
    move_function = functions[0]
    require(
        isinstance(move_function, ast.FunctionDef),
        "move_batch unexpectedly async",
    )

    parameter_names = [argument.arg for argument in move_function.args.args]
    require(
        parameter_names[:2] == ["batch", "device"],
        f"move_batch signature changed: {parameter_names}",
    )

    tensor_to_calls = [
        node
        for node in ast.walk(move_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and node.func.attr == "to"
    ]
    require(
        len(tensor_to_calls) == 1,
        f"move_batch tensor .to call is not unique: "
        f"{[(node.lineno, ast.unparse(node)) for node in tensor_to_calls]}",
    )

    original_to_call = tensor_to_calls[0]
    require(
        isinstance(original_to_call.func.value, ast.Name),
        "move_batch .to receiver is not a simple value name",
    )
    original_receiver = original_to_call.func.value.id

    preserved_to_call = copy.deepcopy(original_to_call)
    preserved_to_call.func.value = ast.Name(id="_value", ctx=ast.Load())
    ast.fix_missing_locations(preserved_to_call)
    tensor_transfer_expression = ast.unparse(preserved_to_call)

    original_function_source = ast.get_source_segment(text, move_function)
    require(
        original_function_source is not None,
        "could not extract original move_batch source",
    )

    corrected_function_source = (
        "def move_batch(batch, device):\n"
        "    # P3 batches contain tensors plus provenance/metadata lists.\n"
        "    def _f7_move_value(_value):\n"
        "        if isinstance(_value, torch.Tensor):\n"
        f"            return {tensor_transfer_expression}\n"
        "        if isinstance(_value, dict):\n"
        "            return {\n"
        "                _key: _f7_move_value(_child)\n"
        "                for _key, _child in _value.items()\n"
        "            }\n"
        "        if isinstance(_value, list):\n"
        "            return [_f7_move_value(_child) for _child in _value]\n"
        "        if isinstance(_value, tuple):\n"
        "            return tuple(\n"
        "                _f7_move_value(_child) for _child in _value\n"
        "            )\n"
        "        return _value\n"
        "\n"
        "    return _f7_move_value(batch)\n"
    )
    ast.parse(corrected_function_source)

    start_offset, end_offset = absolute_span(move_function, offsets)
    corrected = apply_replacements(
        text,
        [(start_offset, end_offset, corrected_function_source)],
    )
    corrected_tree = ast.parse(corrected)
    compile(corrected, str(final_path), "exec")
    ...
    require(
        len(corrected_functions) == 1,
        "corrected move_batch function is not unique",
    )
    corrected_function = corrected_functions[0]
    corrected_source = ast.get_source_segment(corrected, corrected_function)
    require(corrected_source is not None, "corrected move_batch unavailable")
    ...
    require(
        len(corrected_to_calls) == 1,
        "corrected move_batch tensor .to call is not unique",
    )
    require(
        ast.unparse(corrected_to_calls[0]) == tensor_transfer_expression,
        "tensor transfer expression changed",
    )

    required_fragments = [
        "isinstance(_value, torch.Tensor)",
    ...
    require(
        all(fragment in corrected_source for fragment in required_fragments),
        "recursive move_batch branches are incomplete",
    )
    corrected_to_receiver = corrected_to_calls[0].func.value
    require(
        isinstance(corrected_to_receiver, ast.Name)
        and corrected_to_receiver.id == "_value",
        "corrected tensor .to receiver is not _value",
    )
    if original_receiver != "_value":
        legacy_receiver_loads = [
            node
            for node in ast.walk(corrected_function)
            if isinstance(node, ast.Name)
            and isinstance(node.ctx, ast.Load)
            and node.id == original_receiver
        ]
        require(
            not legacy_receiver_loads,
            "legacy unconditional move_batch receiver survived",
        )

    # Protect every module-level function except the deliberately patched
    # move_batch function. The generated trainer has no module-level function
    # named evaluate, so a hard-coded required-name list is invalid.
    def module_level_function_inventory(
        module_text: str,
        ...
        for node in module_tree.body:
            ...
            if node.name in inventory:
                duplicate_names.setdefault(
                    node.name,
                    [inventory[node.name]["line"]],
                ).append(int(node.lineno))
                continue
            ...
            require(
                ...
                f"could not extract module-level function {node.name}",
            )
            inventory[node.name] = {
                "line": int(node.lineno),
                "kind": type(node).__name__,
                "source": function_source,
                "sha256": sha256_text(function_source),
            }

        require(
            not duplicate_names,
            "duplicate module-level function names: "
            f"{duplicate_names}",
        )
        return inventory

    original_function_inventory = module_level_function_inventory(
        text,
        tree,
    )
    corrected_function_inventory = module_level_function_inventory(
        corrected,
        corrected_tree,
    )

    require(
        set(original_function_inventory)
        == set(corrected_function_inventory),
        "module-level function-name set changed during move_batch patch: "
        f"before={sorted(original_function_inventory)}; "
        f"after={sorted(corrected_function_inventory)}",
    )
    require(
        "move_batch" in original_function_inventory,
        "move_batch missing from original function inventory",
    )

    changed_function_names = sorted(
        name
        for name in original_function_inventory
        if (
            original_function_inventory[name]["source"]
            != corrected_function_inventory[name]["source"]
        )
    )
    require(
        changed_function_names == ["move_batch"],
        "unexpected module-level function source changed: "
        f"{changed_function_names}",
    )

    unchanged_function_names = sorted(
        name
        for name in original_function_inventory
        if name != "move_batch"
    )
    require(
        all(
            original_function_inventory[name]["sha256"]
            == corrected_function_inventory[name]["sha256"]
            for name in unchanged_function_names
        ),
        "one or more non-move_batch function hashes changed",
    )

    atomic_text(final_path, corrected, mode=0o555)

    return {
        "input_item_batch_trainer": str(item_batch_trainer_path),
        "input_item_batch_trainer_sha256": sha256_file(
            item_batch_trainer_path
        ),
        "final_recursive_transfer_trainer": str(final_path),
        "final_recursive_transfer_trainer_sha256": sha256_file(final_path),
        "failure_classification": (
            "P3_default_collate_preserved_non_tensor_metadata_as_list_but_"
            "legacy_P2_move_batch_called_to_on_every_top_level_value"
        ),
        "original_move_batch_source": original_function_source,
        "original_move_batch_sha256": sha256_text(original_function_source),
        "corrected_move_batch_source": corrected_source,
        "corrected_move_batch_sha256": sha256_text(corrected_source),
        "original_to_receiver": original_receiver,
        "preserved_tensor_transfer_expression": tensor_transfer_expression,
        "tensor_transfer_expression_preserved": True,
        "tensor_leaves_moved": True,
        "dict_structure_preserved": True,
        "list_structure_preserved": True,
        "tuple_structure_preserved": True,
        "non_tensor_leaves_preserved": True,
        "default_collate_preserved": True,
        "function_source_resolution": (
            "complete_module_level_function_inventory"
        ),
        "hard_coded_evaluate_requirement_removed": True,
        "module_level_function_names_before": sorted(
            original_function_inventory
        ),
        "module_level_function_names_after": sorted(
            corrected_function_inventory
        ),
        "changed_module_level_functions": changed_function_names,
        "unchanged_module_level_functions": unchanged_function_names,
        "all_non_move_batch_functions_unchanged": True,
        "compute_loss_unchanged": (
            "compute_loss" in unchanged_function_names
        ),
        "train_one_epoch_unchanged": (
            "train_one_epoch" in unchanged_function_names
        ),
        "optimizer_scheduler_clipping_not_targeted": True,
        "early_stop_not_targeted": True,
        "model_architecture_not_targeted": True,
...
def exact_function_source(
    ...
    """Return one module-level function without confusing nested helpers.

    Scientific functions such as compute_loss, train_one_epoch, evaluate, and
    move_batch are module-level trainer functions. ast.walk(tree) also visits
    nested local functions, so it can falsely report non-uniqueness when a
    local helper happens to reuse a name such as evaluate.
    """
    module_matches = [
        node
        for node in tree.body
    ...
    nested_matches = [
        ...
        if node not in module_matches
    ]

    require(
        len(module_matches) == 1,
        f"{name} module-level function is not unique: "
        f"module_level={[(node.lineno, type(node).__name__) for node in module_matches]}; "
        f"nested={[(node.lineno, type(node).__name__) for node in nested_matches]}",
    )
    source = ast.get_source_segment(text, module_matches[0])
    require(
        source is not None,
        f"could not extract module-level {name}",
    )
    return source
...
def main() -> int:
    ...
    inventory_self_test_before = ast.parse(
        "def compute_loss():\n"
        "    return 1\n"
        "\n"
        "def train_one_epoch():\n"
    ...
    inventory_self_test_after = ast.parse(
        "def compute_loss():\n"
        "    return 1\n"
        "\n"
        "def train_one_epoch():\n"
    ...
    before_names = sorted(
        node.name
        for node in inventory_self_test_before.body
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))
    )
    after_names = sorted(
        node.name
        for node in inventory_self_test_after.body
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))
    )
    require(
        before_names == after_names
        == ["compute_loss", "move_batch", "train_one_epoch"],
        "complete function-inventory self-test failed",
    )
    ...
    legacy_guard_corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r6_r2_legacy_guards.py"
    )
    legacy_guard_patch = create_corrected_legacy_guard_trainer(
    ...
    item_batch_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r6_r2_item_batch.py"
    )
    item_batch_patch = create_item_batch_route_trainer(
        legacy_guard_corrected_trainer_path,
        item_batch_trainer_path,
        sha256_file(legacy_guard_corrected_trainer_path),
    )
    ...
    batch_device_transfer_patch = (
        create_recursive_batch_device_transfer_trainer(
            item_batch_trainer_path,
            corrected_trainer_path,
            sha256_file(item_batch_trainer_path),
        )
    ...
    move_batch_source = exact_function_source(
        trainer_text,
        trainer_tree,
        "move_batch",
    )
    require(
        "isinstance(_value, torch.Tensor)" in move_batch_source,
        "recursive tensor-aware move_batch missing from final trainer",
    )
    require(
        "isinstance(_value, list)" in move_batch_source,
        "list-preserving move_batch branch missing",
    )

    corrected_integer_constants = [
        node.value
    ...
    surviving_legacy_guard_values = sorted(
        value
        for value in (70166, 12528, 43273)
        if value in corrected_integer_constants
    )
    require(
        not surviving_legacy_guard_values,
        f"legacy guard values survived in corrected trainer: "
        f"{surviving_legacy_guard_values}",
    )
    require(
        all(
            value in corrected_integer_constants
            for value in (110855, 13863, 60553)
        ),
        "one or more certified P3 guard values are absent from corrected trainer",
    )
    ...
    scratch = output_dir / "F7_E2_R6_R2_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    require(
        isinstance(preview_batch, dict),
        f"default-collated preview batch is not a dict: "
        f"{type(preview_batch).__name__}",
    )

    def describe_batch_value(value: Any) -> dict[str, Any]:
        row = {"python_type": type(value).__name__}
        if isinstance(value, torch.Tensor):
            row.update({
                "kind": "tensor",
            ...
            row.update({
                "kind": "dict",
                "children": {
                    str(key): describe_batch_value(child)
                    for key, child in value.items()
                },
            })
            ...
            row.update({
                ...
                "length": len(value),
                "child_types": sorted({
                    type(child).__name__ for child in value
                }),
            })
            ...
            row.update({"kind": "non_tensor_leaf"})
        return row

    preview_batch_schema = {
        str(key): describe_batch_value(value)
        for key, value in preview_batch.items()
    }
    preview_non_tensor_keys = sorted(
        str(key)
        for key, value in preview_batch.items()
        if not isinstance(value, torch.Tensor)
    )
    preview_list_keys = sorted(
        str(key)
        for key, value in preview_batch.items()
        if isinstance(value, list)
    )
    require(
        preview_non_tensor_keys,
        "P3 preview batch unexpectedly has no non-tensor metadata leaves",
    )
    require(
        preview_list_keys,
        "P3 preview batch unexpectedly has no list-valued metadata",
    )
    del preview_loader
    del preview_batch
    ...
    static_results = {
        "recovery_classification": (
            "R6_R1_assumed_a_module_level_evaluate_function_but_the_generated_trainer_has_no_such_function"
        ),
        "prior_AST_numeric_literal_recovery_preserved": True,
        "prior_output_directory_recovery_preserved": True,
        "legacy_guard_patch": legacy_guard_patch,
        "item_batch_patch": item_batch_patch,
        "batch_device_transfer_patch": batch_device_transfer_patch,
    ...
    runtime_results = {
        ...
        "certified_dynamic70_parameter_count": EXPECTED_PARAMETER_COUNT,
        "legacy_parameter_count_guard_value": 43273,
        "dynamic70_parameter_count_guard_recovered": True,
        "recursive_batch_device_transfer_recovered": True,
        "preview_batch_schema": preview_batch_schema,
        "preview_non_tensor_keys": preview_non_tensor_keys,
        "preview_list_keys": preview_list_keys,
    ...
    legacy_guard_patch_path = output_dir / (
        "F7_E2_R6_R2_LEGACY_CARDINALITY_AND_PARAMETER_COUNT_GUARD_PATCH.json"
    )
    atomic_json(legacy_guard_patch_path, legacy_guard_patch)

    item_batch_patch_path = output_dir / (
        "F7_E2_R6_R2_PAIRBLOCK_TO_ITEMBATCH_ROUTE_PATCH.json"
    )
    atomic_json(item_batch_patch_path, item_batch_patch)

    batch_device_transfer_patch_path = output_dir / (
        "F7_E2_R6_R2_RECURSIVE_BATCH_DEVICE_TRANSFER_PATCH.json"
    )
    atomic_json(
        batch_device_transfer_patch_path,
        batch_device_transfer_patch,
    )

    launcher_patch_path = output_dir / (
        "F7_E2_R6_R2_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R6_R2_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R6_R2_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R6_R2_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    decision = {
        ...
        "R5_dynamic70_parameter_count_guard_recovered": True,
        ...
        "recursive_batch_device_transfer_verified": True,
        "complete_module_level_function_inventory_verified": True,
        "nonexistent_evaluate_requirement_removed": True,
        "non_tensor_batch_metadata_preserved": True,
    ...
    report = {
        ...
        "artifacts": {
            ...
            "legacy_guard_patch": str(legacy_guard_patch_path),
            "item_batch_patch": str(item_batch_patch_path),
            "batch_device_transfer_patch": str(batch_device_transfer_patch_path),
        ...
        "provenance": {
            ...
            "original_R5_generated_trainer_sha256": (
                legacy_guard_patch["original_trainer_sha256"]
            ),
            "generated_trainer_sha256": sha256_file(trainer_path),
            "legacy_guard_patch_sha256": sha256_file(legacy_guard_patch_path),
            "item_batch_patch_sha256": sha256_file(item_batch_patch_path),
            "batch_device_transfer_patch_sha256": sha256_file(batch_device_transfer_patch_path),
            "runtime_adapter_sha256": sha256_file(adapter_path),
    ...
    atomic_json(
        canonical_lock_path,
        {
            ...
            "R6_recursive_batch_device_transfer_verified": True,
            "effective_item_batch_size": EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE,
            "train_batches": EXPECTED_TRAIN_BATCHES,
            "validation_batches": EXPECTED_VALIDATION_BATCHES,
            "corrected_trainer_bound_in_matrix_launcher": True,
    ...
    print(
        ...
        f"{legacy_guard_patch['original_trainer']}"
    )
    print(
        "original_R5_generated_trainer_sha256="
        f"{legacy_guard_patch['original_trainer_sha256']}"
    )
    ...
    print("legacy_parameter_count_guard=43273")
    print("certified_dynamic70_parameter_count=60553")
    print("dynamic70_parameter_count_guard_recovered=true")
    ...
    print("recursive_batch_device_transfer=true")
    print(
        "function_source_resolution="
        "complete_module_level_function_inventory"
    )
    print("hard_coded_evaluate_requirement_removed=true")
    print("only_move_batch_may_change=true")
    print("complete_function_inventory_self_test=true")
    print("tensor_leaves_moved=true")
    print("non_tensor_metadata_preserved=true")
    print(f"preview_non_tensor_keys={preview_non_tensor_keys}")
    print(f"preview_list_keys={preview_list_keys}")
    ...
    print(f"legacy_guard_patch={legacy_guard_patch_path}")
    print(f"item_batch_patch={item_batch_patch_path}")
    print(f"batch_device_transfer_patch={batch_device_transfer_patch_path}")
