...
STAGE = "V5_P3_F7_E2_R4_R1_R1_FROZEN_BATCH_GEOMETRY_RECOVERY"
...
def create_item_batch_route_trainer(
    ...
    require(
        ...
        f"sampler variables remain active after patch: "
        f"{sampler_name_loads}",
    )
    ...
    require(
        corrected_loader_rows["train_loader"]["generator_present"] is True,
        "corrected train loader lacks deterministic generator",
    )
    require(
...
def main() -> int:
    ...
    cardinality_corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4_r1_cardinality.py"
    )
    ...
    corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4_r1.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r4_r1.sh"
    )
    require(
        "shlex" in globals(),
    ...
    scratch = output_dir / "F7_E2_R4_R1_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "R4_assumed_semantic_pair_count_parameter_name_but_sampler_signature_uses_a_neutral_parameter_name"
        ),
    ...
    cardinality_patch_path = output_dir / (
        "F7_E2_R4_R1_DATASET_CARDINALITY_GUARD_PATCH.json"
    )
    atomic_json(cardinality_patch_path, cardinality_patch)

    item_batch_patch_path = output_dir / (
        "F7_E2_R4_R1_PAIRBLOCK_TO_ITEMBATCH_ROUTE_PATCH.json"
    )
    atomic_json(item_batch_patch_path, item_batch_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R4_R1_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R4_R1_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R4_R1_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R4_R1_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    atomic_json(
        canonical_lock_path,
        {
            ...
            "item_batch_patch_sha256": sha256_file(item_batch_patch_path),
            "R3A_dataset_cardinality_guard_recovered": True,
            "R4_R1_pair_block_sampler_removed": True,
            "R4_R1_item_level_batch_route_verified": True,
            "effective_item_batch_size": EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE,
            "train_batches": EXPECTED_TRAIN_BATCHES,
            "validation_batches": EXPECTED_VALIDATION_BATCHES,
            "corrected_trainer_bound_in_matrix_launcher": True,
