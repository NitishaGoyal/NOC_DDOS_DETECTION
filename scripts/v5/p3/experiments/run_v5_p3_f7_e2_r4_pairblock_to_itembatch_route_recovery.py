...
STAGE = "V5_P3_F7_E2_R4_PAIRBLOCK_TO_ITEMBATCH_ROUTE_RECOVERY"
...
def create_item_batch_route_trainer(
    ...
    """Replace the inherited pair-block sampler with item-level batching.

    The legacy V5-P2 dataset stored adjacent CONTROL/ATTACK members in pairs.
    PairBlockBatchSampler therefore:
      - required an even dataset length;
      - batched a fixed number of pairs;
      - yielded 2 * pairs_per_batch items per optimizer step.

    The P3 guarded split is an item-level dataset and has an odd training
    cardinality (110,855). Pair adjacency is not a valid runtime invariant.

    This patch preserves the *effective item batch size* and shuffle policy,
    while removing the even-cardinality/pair-adjacency requirement.
    """
    require(
    ...
    def class_init_signature(class_name: str):
        ...
        return names, default_map

    sampler_parameter_names, sampler_defaults = class_init_signature(
        "PairBlockBatchSampler"
    )

    def call_parameter(call: ast.Call, parameter: str):
        for keyword in call.keywords:
            if keyword.arg == parameter:
                return keyword.value
        if parameter in sampler_parameter_names:
            index = sampler_parameter_names.index(parameter)
            if index < len(call.args):
                return call.args[index]
        return sampler_defaults.get(parameter)
    ...
    # Resolve the pair-count parameter from the actual class signature.
    candidate_pair_parameters = [
        name
        for name in sampler_parameter_names
    ...
    ordered_pair_parameters = (
        [name for name in preferred_names if name in sampler_parameter_names]
        + [
            name for name in candidate_pair_parameters
            if name not in preferred_names
        ]
    )
    require(
        ordered_pair_parameters,
        "could not identify the PairBlockBatchSampler pair-count parameter",
    )
    pair_parameter = ordered_pair_parameters[0]
    ...
    for target in sampler_targets:
        ...
        pair_node = call_parameter(value, pair_parameter)
        pair_count = evaluated_constant(pair_node)
        require(
            isinstance(pair_count, int)
            and not isinstance(pair_count, bool)
            and pair_count > 0,
            f"{target} pair count is not a positive integer constant: "
            f"{ast.unparse(pair_node) if pair_node is not None else None}",
        )

        effective_item_batch_size = 2 * pair_count
        require(
            effective_item_batch_size
            == EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE,
            f"{target} effective item batch size changed: "
            f"2 * {pair_count} = {effective_item_batch_size}, "
            f"expected {EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE}",
        )

        shuffle_node = call_parameter(value, "shuffle")
        shuffle_value = evaluated_constant(shuffle_node)
        if shuffle_value is None:
            # Preserve the established route if the class did not expose an
            # explicit shuffle argument.
            shuffle_value = target == "train_sampler"
            shuffle_source = repr(shuffle_value)
            shuffle_resolution = "target_role_default"
        else:
            require(
                isinstance(shuffle_value, bool),
                f"{target} shuffle policy is not boolean: "
                f"{ast.unparse(shuffle_node)}",
            )
            shuffle_source = ast.unparse(shuffle_node)
            shuffle_resolution = "sampler_call_or_default"

        seed_node = call_parameter(value, "seed")
        seed_source = (
            ast.unparse(seed_node)
            if seed_node is not None
            else "seed"
        )

        value_start, value_end = absolute_span(value, offsets)
        replacements.append((value_start, value_end, "None"))

        sampler_rows[target] = {
            "assignment_line": int(assignment.lineno),
            "original_call": ast.unparse(value),
            "pair_count_parameter": pair_parameter,
            "pairs_per_batch": pair_count,
            "effective_item_batch_size": effective_item_batch_size,
    ...
    for loader_target, sampler_target in loader_targets.items():
        ...
        require(
            isinstance(value, ast.Call)
            and call_name(value.func).rsplit(".", 1)[-1] == "DataLoader",
            f"{loader_target} is not a DataLoader call: {ast.unparse(value)}",
        )
        ...
        if shuffle_value:
            ...
            generator_expression = ast.parse(
                f"torch.Generator().manual_seed({seed_source})",
                mode="eval",
            ).body
    ...
    require(
        ...
        f"active PairBlockBatchSampler calls survived: {active_pair_calls}",
    )
    ...
    sampler_name_loads = {
        ...
        for name in sampler_targets
    }
    require(
        all(not rows for rows in sampler_name_loads.values()),
        f"sampler variables remain active after patch: {sampler_name_loads}",
    )
    ...
    # Inventory all remaining pair-labelled source sites. Definitions and
    # provenance text may remain, but no active pair-block construction or
    # sampler-variable load is permitted.
    ...
    return {
        ...
        "failure_classification": (
            "inherited_P2_PairBlockBatchSampler_requires_even_pair_aligned_"
            "dataset_but_P3_split_is_item_level_and_train_count_is_odd"
        ),
        "sampler_class": "PairBlockBatchSampler",
        ...
        "active_sampler_variable_load_count": 0,
        "effective_item_batch_size": (
            EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE
        ),
        "effective_item_batch_size_derivation": (
            "2 * inherited pairs_per_batch"
        ),
        ...
        "deterministic_train_shuffle_generator": (
            corrected_loader_rows["train_loader"]["generator_present"]
        ),
...
def main() -> int:
    ...
    cardinality_corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4_cardinality.py"
    )
    ...
    corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r4.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r4.sh"
    )
    require(
        "shlex" in globals(),
    ...
    scratch = output_dir / "F7_E2_R4_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "inherited_P2_PairBlockBatchSampler_requires_even_pair_aligned_dataset_but_P3_split_is_item_level_and_train_count_is_odd"
        ),
    ...
    cardinality_patch_path = output_dir / (
        "F7_E2_R4_DATASET_CARDINALITY_GUARD_PATCH.json"
    )
    atomic_json(cardinality_patch_path, cardinality_patch)

    item_batch_patch_path = output_dir / (
        "F7_E2_R4_PAIRBLOCK_TO_ITEMBATCH_ROUTE_PATCH.json"
    )
    atomic_json(item_batch_patch_path, item_batch_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R4_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R4_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R4_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R4_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    atomic_json(
        canonical_lock_path,
        {
            ...
            "item_batch_patch_sha256": sha256_file(item_batch_patch_path),
            "R3A_dataset_cardinality_guard_recovered": True,
            "R4_pair_block_sampler_removed": True,
            "R4_item_level_batch_route_verified": True,
            "effective_item_batch_size": EXPECTED_EFFECTIVE_ITEM_BATCH_SIZE,
            "train_batches": EXPECTED_TRAIN_BATCHES,
            "validation_batches": EXPECTED_VALIDATION_BATCHES,
            "corrected_trainer_bound_in_matrix_launcher": True,
