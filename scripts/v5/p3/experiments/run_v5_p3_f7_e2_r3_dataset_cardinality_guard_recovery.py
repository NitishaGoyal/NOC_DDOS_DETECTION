...
STAGE = "V5_P3_F7_E2_R3_DATASET_CARDINALITY_GUARD_RECOVERY"
...
def create_corrected_cardinality_trainer(
    ...
    """Patch only the two frozen legacy P2 split cardinalities.

    The generated F7 trainer correctly uses the guarded P3 adapters, but the
    inherited P2 execution skeleton still holds when it sees anything other
    than 70,166 train and 12,528 validation items. F7's certified fixed split
    is 110,855 train and 13,863 validation items.
    """
    require(
    ...
    for old_value, new_value in replacements.items():
        pattern = re.compile(rf"(?<![0-9]){old_value}(?![0-9])")
        matches = list(pattern.finditer(original))
        require(
            matches,
            f"legacy cardinality {old_value} not found in generated trainer",
        )
        require(
            len(matches) <= 20,
            f"unexpectedly many occurrences of legacy cardinality "
            f"{old_value}: {len(matches)}",
        )

        token_counts[str(old_value)] = len(matches)
        ...
        original = pattern.sub(str(new_value), original)

    corrected = original
    corrected_tree = ast.parse(corrected)
    compile(corrected, str(corrected_path), "exec")

    corrected_source = ast.unparse(corrected_tree)
    require(
        "70166" not in corrected_source,
        "legacy train cardinality survived correction",
    )
    require(
        "12528" not in corrected_source,
        "legacy validation cardinality survived correction",
    )
    require(
        "110855" in corrected_source,
        "certified P3 train cardinality missing after correction",
    )
    require(
        "13863" in corrected_source,
        "certified P3 validation cardinality missing after correction",
    )
    ...
    return {
        ...
        "token_replacement_counts": token_counts,
        "exact_source_contexts": contexts,
        "patch_scope": (
            "exact standalone integer-token replacement only; "
...
def main() -> int:
    ...
    corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r3.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r3.sh"
    )
    ...
    scratch = output_dir / "F7_E2_R3_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "E2_R2_inherited_P2_split_cardinality_guards"
        ),
    ...
    cardinality_patch_path = output_dir / (
        "F7_E2_R3_DATASET_CARDINALITY_GUARD_PATCH.json"
    )
    atomic_json(cardinality_patch_path, cardinality_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R3_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R3_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R3_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R3_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
    ...
    decision = {
        ...
        "R3_dataset_cardinality_guard_recovered": True,
    ...
    print("cardinality_patch_scope=exact_standalone_integer_tokens_only")
    print(
        "canonical_state_hash_algorithm="
        "sorted_state_dict_key_dtype_shape_and_contiguous_cpu_bytes"
    )
