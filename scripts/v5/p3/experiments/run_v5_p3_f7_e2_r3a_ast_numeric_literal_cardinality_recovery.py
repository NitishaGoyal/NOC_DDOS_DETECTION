...
STAGE = "V5_P3_F7_E2_R3A_AST_NUMERIC_LITERAL_CARDINALITY_RECOVERY"
...
def main() -> int:
    ...
    corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r3a.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r3a.sh"
    )
    ...
    scratch = output_dir / "F7_E2_R3A_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    cardinality_patch_path = output_dir / (
        "F7_E2_R3A_DATASET_CARDINALITY_GUARD_PATCH.json"
    )
    atomic_json(cardinality_patch_path, cardinality_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R3A_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R3A_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R3A_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R3A_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
