...
RECOVERY_STAGE = "V5_P3_F4_R1_INSTALLED_SCRIPT_CLI_RECOVERY"
...
def option_kind(names: list[str]) -> str:
    ...
    if any(token in joined for token in (
        ...
        return "a5_dir"
    if any(token in joined for token in (
        "installed_script", "installed-script"
    )):
...
def main() -> int:
    ...
    atomic_json(
        invocation_contract_path,
        {
            ...
            "recovery": {
                ...
                "previous_failure": (
                    "pre-execution CLI mapping rejected the required "
                    "--installed-script argument"
                ),
                "mapping_correction": (
                    "--installed-script is mapped to the exact frozen "
                    "official D1 exporter path"
                ),
                "previous_validation_execution_started": False,
            },
        },
    )
    ...
    report = {
        ...
        "recovery": {
            "stage": RECOVERY_STAGE,
            "failure_classification": "pre_execution_cli_mapping_only",
            "previous_validation_execution_started": False,
            "previous_validation_tensors_loaded": False,
            "correction": (
                "Map the D1 exporter's required --installed-script argument "
                "to the hash-verified official D1 exporter path."
            ),
            "scientific_computation_reused": False,
            "actual_F4_replay_executed_by_this_run": True,
        },
        "scope": (
    ...
    print("failure_classification=pre_execution_cli_mapping_only")
    print("previous_validation_execution_started=false")
    print("installed_script_mapping=official_D1_exporter_path")
