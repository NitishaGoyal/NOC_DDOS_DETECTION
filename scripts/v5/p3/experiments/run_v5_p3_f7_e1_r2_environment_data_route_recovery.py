...
STAGE = "V5_P3_F7_E1_R2_ENVIRONMENT_DATA_ROUTE_RECOVERY"
CAMPAIGN = "V5-P3-F0-D70-FEATURE-STUDY"
CLASSIFICATION = "TRAIN-DERIVED"
FAILURE_CLASSIFICATION = (
    "E1_R2_data_cli_not_present_dataset_route_is_environment_injected"
)
...
RUNTIME_ADAPTER_RELATIVE_PATH = Path(
    "src/models/v5_p3_f7_reconstructed_runtime_adapter_e1_r2.py"
)
...
def _build_fresh_model() -> torch.nn.Module:
    ...
    f6 = _import_source(
        f6_source,
        "_v5_p3_f7_e1_r2_runtime_model_capture",
    )
...
class _F7FixedSplitDatasetAdapter(Dataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loader = _import_source(
            loader_path,
            f"_v5_p3_f7_e1_r2_loader_{self.FIXED_SPLIT}",
        )
...
for SPEC in "$MATRIX_DIR"/*.json; do
  ...
  CMD=(
    ...
    "$TRAINER"
    {seed_option} "107"
{data_fragment}    {output_option} "$RUN_DIR"
  )
{device_fragment}
  printf 'launching %s\\n' "$LABEL"
  printf 'command:'
...
def main() -> int:
    ...
    import_text = (
        "from src.models.v5_p3_f7_reconstructed_runtime_adapter_e1_r2 "
        "import (\n"
    ...
    generated_text = (
        "# AUTO-GENERATED BY V5-P3 F7-E1-R2.\n"
    ...
    matrix_dir = f7_root / "F7_E1_R2_PRIMARY_SEED107_RUN_SPECS"
    ...
    route_inventory_path = output_dir / (
        "F7_E1_R2_CONSTRUCTOR_ASSIGNMENT_ROUTE_INVENTORY.json"
    )
    patch_map_path = output_dir / (
        "F7_E1_R2_GENERATED_SOURCE_AND_CALLSITE_PATCH_MAP.json"
    )
    matrix_manifest_path = output_dir / (
        "F7_E1_R2_PRIMARY_SEED107_MATRIX_MANIFEST.json"
    )
    cli_map_path = output_dir / (
        "F7_E1_R2_GENERATED_TRAINER_CLI_BINDING_MAP.json"
    )
    authorization_path = output_dir / (
        "F7_E1_R2_E2_PREFLIGHT_AUTHORIZATION.json"
    )
