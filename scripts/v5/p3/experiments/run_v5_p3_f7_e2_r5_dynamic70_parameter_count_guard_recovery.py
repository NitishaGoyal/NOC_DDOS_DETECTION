...
STAGE = "V5_P3_F7_E2_R5_R1_FROZEN_BATCH_GEOMETRY_RECOVERY"
...
def create_corrected_legacy_guard_trainer(
    ...
    return {
        ...
        "failure_classification": (
            ...
            "guard_43273_while_Dynamic70_has_60553_parameters"
        ),
...
def main() -> int:
    ...
    legacy_guard_corrected_trainer_path = (
        ...
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r5_legacy_guards.py"
    )
    ...
    corrected_trainer_path = (
        original_trainer_path.parent
        / "run_v5_p3_f7_reconstructed_group_ablation_trainer_e2_r5.py"
    )
    ...
    corrected_launcher_path = (
        ...
        / "run_v5_p3_f7_primary_seed107_matrix_e2_r5.sh"
    )
    require(
        "shlex" in globals(),
    ...
    scratch = output_dir / "F7_E2_R5_RUNTIME_PREFLIGHT_SCRATCH"
    if scratch.exists():
    ...
    static_results = {
        "recovery_classification": (
            "instrumented_trainer_retained_legacy_P2_parameter_count_guard_43273_while_Dynamic70_has_60553_parameters"
        ),
    ...
    legacy_guard_patch_path = output_dir / (
        "F7_E2_R5_LEGACY_CARDINALITY_AND_PARAMETER_COUNT_GUARD_PATCH.json"
    )
    atomic_json(legacy_guard_patch_path, legacy_guard_patch)

    item_batch_patch_path = output_dir / (
        "F7_E2_R5_PAIRBLOCK_TO_ITEMBATCH_ROUTE_PATCH.json"
    )
    atomic_json(item_batch_patch_path, item_batch_patch)

    launcher_patch_path = output_dir / (
        "F7_E2_R5_CORRECTED_MATRIX_LAUNCHER_PATCH.json"
    )
    atomic_json(launcher_patch_path, launcher_patch)

    static_path = output_dir / (
        "F7_E2_R5_GENERATED_TRAINER_STATIC_CERTIFICATION.json"
    )
    runtime_path = output_dir / (
        "F7_E2_R5_RUNTIME_ADAPTER_AND_FIRST_BATCH_CERTIFICATION.json"
    )
    decision_path = output_dir / (
        "F7_E2_R5_PRIMARY_MATRIX_EXECUTION_DECISION.json"
    )
