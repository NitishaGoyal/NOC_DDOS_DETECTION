...
RECOVERY_STAGE = "V5_P3_F4M_R1_SIDE_EFFECT_FREE_SELECTION_SCORE_RECOVERY"
...
def _safe_source_namespace(
    path: Path,
    required_function: str,
) -> SimpleNamespace:
    """
    Execute only a side-effect-free AST subset of a Python source file.

    Included:
    - safe standard/scientific imports;
    - literal top-level constants;
    - function definitions with decorators removed.

    Excluded:
    - argparse construction;
    - top-level function calls;
    - main() execution;
    - dataset/model/checkpoint access.
    """
    import ast
    import copy
...
def main() -> int:
    ...
    report = {
        ...
        "recovery": {
            ...
            "failure_classification": (
                "candidate_source_import_side_effect_before_certification"
            ),
            "previous_failure": (
                "An imported selection-score candidate executed top-level "
                "argparse and demanded --data-link."
            ),
            "correction": (
                "Probe and invoke official selection-score functions through "
                "a side-effect-free AST subset containing only safe imports, "
                "literal constants, and function definitions."
            ),
            "full_candidate_module_import_authorized": False,
            "validation_dataset_access_used": False,
        },
        "scope": (
    ...
    print("candidate_full_module_import=false")
    print("candidate_safe_AST_subset=true")
