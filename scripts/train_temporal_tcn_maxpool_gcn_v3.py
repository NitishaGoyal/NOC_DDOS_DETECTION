...
class TemporalResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        residual = x
        h = self.dropout(F.relu(self.norm1(self.conv1(x))))
...
class TemporalTCNEncoder(nn.Module):
    """
    TCN temporal encoder with max pooling instead of attention pooling.

    Purpose:
    - keep residual/dilated temporal modeling
    - remove softmax attention pooling
    - reduce sensitivity to short benign bursts
    - make hardware implementation simpler
    """

    def __init__(self, input_features, temporal_dim, dropout=0.1, tcn_dropout=None, **kwargs):
        super().__init__()

        if tcn_dropout is not None:
            dropout = tcn_dropout

        self.input_proj = nn.Conv1d(input_features, temporal_dim, kernel_size=1)

        self.conv1a = nn.Conv1d(temporal_dim, temporal_dim, kernel_size=3, padding=1, dilation=1)
        self.bn1a = nn.BatchNorm1d(temporal_dim)
        self.drop1a = nn.Dropout(dropout)
        self.conv1b = nn.Conv1d(temporal_dim, temporal_dim, kernel_size=3, padding=1, dilation=1)
        self.bn1b = nn.BatchNorm1d(temporal_dim)

        self.conv2a = nn.Conv1d(temporal_dim, temporal_dim, kernel_size=3, padding=2, dilation=2)
        self.bn2a = nn.BatchNorm1d(temporal_dim)
        self.drop2a = nn.Dropout(dropout)
        self.conv2b = nn.Conv1d(temporal_dim, temporal_dim, kernel_size=3, padding=2, dilation=2)
        self.bn2b = nn.BatchNorm1d(temporal_dim)

    def _block(self, x, conv_a, bn_a, drop_a, conv_b, bn_b):
        residual = x
        h = conv_a(x)
        h = bn_a(h)
        h = torch.relu(h)
    ...
    def forward(self, x):
        # x: [B, N, T, F]
        bsz, num_nodes, timesteps, feat_dim = x.shape

        h = x.reshape(bsz * num_nodes, timesteps, feat_dim)
        h = h.transpose(1, 2)  # [B*N, F, T]

        h = self.input_proj(h)
        h = torch.relu(h)

        h = self._block(h, self.conv1a, self.bn1a, self.drop1a, self.conv1b, self.bn1b)
        h = self._block(h, self.conv2a, self.bn2a, self.drop2a, self.conv2b, self.bn2b)

        # Max pooling over time instead of attention pooling
        h = torch.max(h, dim=-1).values  # [B*N, temporal_dim]

        return h.reshape(bsz, num_nodes, -1)



class TemporalTCNGCN(nn.Module):
    def __init__(self, input_features: int, temporal_dim: int = 8, gcn_hidden: int = 16, gcn_out: int = 8, tcn_dropout: float = 0.1):
        super().__init__()
        self.temporal = TemporalTCNEncoder(input_features, temporal_dim, kernel_size=3, dropout=tcn_dropout)
...
def main():
    ...
    parser.add_argument("--out-dir", default="models/temporal_tcn_gcn_early_stop")
    ...
    model = TemporalTCNGCN(input_features=input_feature_dim, temporal_dim=args.temporal_dim, gcn_hidden=args.gcn_hidden, gcn_out=args.gcn_out, tcn_dropout=args.tcn_dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improvement > args.min_delta:
            ...
            checkpoint = {
                ...
                "model_name": "TCN-MaxPool-TemporalGCN",
            }
    ...
    print("\nFINAL")
    print("model:", "TCN-MaxPool-TemporalGCN")
