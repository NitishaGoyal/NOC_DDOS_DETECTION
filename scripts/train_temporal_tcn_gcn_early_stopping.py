...
class TemporalTCNEncoder(nn.Module):
    def __init__(self, in_features: int = 2, embedding_dim: int = 8, kernel_size: int = 3, dropout: float = 0.1):
        super().__init__()
        self.input_projection = nn.Conv1d(in_features, embedding_dim, kernel_size=1)
        self.block1 = TemporalResidualBlock(embedding_dim, kernel_size, dilation=1, dropout=dropout)
        self.block2 = TemporalResidualBlock(embedding_dim, kernel_size, dilation=2, dropout=dropout)
        self.attention_score = nn.Conv1d(embedding_dim, 1, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.block1(h)
        h = self.block2(h)
        attention_weights = torch.softmax(self.attention_score(h), dim=2)
        h = torch.sum(h * attention_weights, dim=2)
