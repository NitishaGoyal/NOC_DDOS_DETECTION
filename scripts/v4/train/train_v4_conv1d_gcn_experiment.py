#!/usr/bin/env python3
"""
V4 experiment-controlled Conv1D-TemporalGCN memmap trainer.

Modes
-----
source-audit       Verify source hash and the explicitly authorized experiment architecture.
...
BASE_MODEL_NAME = "Conv1D-TemporalGCN"
EXPERIMENT_REGISTRY = {
    "a1_frozen": {
        "stage_label": "V4-A1",
        "model_name": "Conv1D-TemporalGCN-A1-Frozen",
        "temporal_dim": 8,
        "gcn_hidden": 16,
        "gcn_out": 8,
        "parameter_count": 882,
    ...
    "a2_capacity_lift": {
        "stage_label": "V4-A2",
        "model_name": "Conv1D-TemporalGCN-A2-CapacityLift",
        "temporal_dim": 16,
        "gcn_hidden": 32,
        "gcn_out": 16,
        "parameter_count": 2274,
    },
}


def experiment_spec(args: argparse.Namespace) -> dict[str, Any]:
    try:
        spec = dict(EXPERIMENT_REGISTRY[args.experiment_designation])
    except KeyError as exc:
        raise RuntimeError(
            f"Unknown experiment designation: {args.experiment_designation!r}"
        ) from exc
    expected = (
        spec["parameter_count"]
        if args.expected_parameter_count is None
        else int(args.expected_parameter_count)
    )
    if expected != int(spec["parameter_count"]):
        raise RuntimeError(
            "Expected-parameter-count authorization mismatch: "
            f"designation={args.experiment_designation} "
            f"registry={spec['parameter_count']} cli={expected}"
        )
    actual_dims = (int(args.temporal_dim), int(args.gcn_hidden), int(args.gcn_out))
    registry_dims = (
        int(spec["temporal_dim"]),
        int(spec["gcn_hidden"]),
        int(spec["gcn_out"]),
    )
    if actual_dims != registry_dims:
        raise RuntimeError(
            "Architecture authorization mismatch: "
            f"designation={args.experiment_designation} "
            f"registry_dims={registry_dims} cli_dims={actual_dims}"
        )
    spec["expected_parameter_count"] = expected
    spec["experiment_designation"] = args.experiment_designation
    return spec
...
# ---------------------------------------------------------------------------
# Shared Conv1D-GCN model classes; dimensions are authorized by EXPERIMENT_REGISTRY.
# ---------------------------------------------------------------------------
...
def model_signature(
    ...
    return {
        ...
        "experiment_designation": spec["experiment_designation"],
...
def architecture_text(signature: dict[str, Any]) -> str:
    conv = signature["temporal_conv"]
    g1 = signature["gcn1"]
    g2 = signature["gcn2"]
    nh = signature["node_head"]
    gh = signature["graph_head"]
    return f"""{signature['stage_label']} explicitly authorized architecture
experiment_designation: {signature['experiment_designation']}
model_name: {signature['model_name']}
parameter_count: {signature['parameter_count']}
expected_parameter_count: {signature['expected_parameter_count']}
input: [B,16,8,24]
temporal: Conv1d({conv['in_channels']},{conv['out_channels']},kernel_size={conv['kernel_size'][0]},stride={conv['stride'][0]},padding={conv['padding'][0]}) -> ReLU -> max over time
gcn1: normalized adjacency, Linear({g1['in_features']},{g1['out_features']}) -> ReLU
gcn2: normalized adjacency, Linear({g2['in_features']},{g2['out_features']}) -> ReLU
node_head: Linear({nh['in_features']},{nh['out_features']}) per node -> [B,16]
graph_readout: mean over 16 routers
graph_head: Linear({gh['in_features']},{gh['out_features']}) -> [B]
"""
...
def source_audit(args: argparse.Namespace) -> int:
    ...
    signature = model_signature(model, spec)
    architecture_ok = (
        signature["parameter_count_ok"]
        and signature["temporal_conv"]["in_channels"] == 24
        and signature["temporal_conv"]["out_channels"] == spec["temporal_dim"]
        and signature["temporal_conv"]["kernel_size"] == [3]
        and signature["temporal_conv"]["padding"] == [1]
        and signature["gcn1"] == {
            "in_features": spec["temporal_dim"],
            "out_features": spec["gcn_hidden"],
        }
        and signature["gcn2"] == {
            "in_features": spec["gcn_hidden"],
            "out_features": spec["gcn_out"],
        }
    ...
    report = {
        "stage": f"{spec['stage_label']}.0 source and architecture audit",
        ...
        "note": (
            ...
            "logic. The model is gated by the explicit experiment registry, semantic "
            "module signature, and exact expected parameter count; the class-level "
            "textual diff is retained for inspection."
        ),
    }
    json_dump(out / f"{args.experiment_designation}_source_audit.json", report)
    ...
    print(f"{spec['stage_label'].replace('-', '_')}_SOURCE_AUDIT_PASS" if report["pass"] else f"{spec['stage_label'].replace('-', '_')}_SOURCE_AUDIT_FAIL")
...
def loader_preflight(args: argparse.Namespace) -> int:
    ...
    print(f"{spec['stage_label'].replace('-', '_')}_LOADER_PREFLIGHT_PASS" if checks["pass"] else f"{spec['stage_label'].replace('-', '_')}_LOADER_PREFLIGHT_FAIL")
...
def prepare_training_objects(
    ...
    signature = model_signature(model, spec)
    if signature["parameter_count"] != signature["expected_parameter_count"]:
...
def one_batch_preflight(args: argparse.Namespace) -> int:
    ...
    expected_shapes = {
        ...
        "temporal_embedding": [args.batch_size, 16, args.temporal_dim],
        "gcn1_output": [args.batch_size, 16, args.gcn_hidden],
        "gcn2_output": [args.batch_size, 16, args.gcn_out],
        "node_logits": [args.batch_size, 16],
        "graph_embedding": [args.batch_size, args.gcn_out],
    ...
    report = {
        "stage": f"{objects['experiment_spec']['stage_label']}.2 one-batch forward/backward preflight",
        "device": str(device),
    ...
    report["pass"] = bool(
        ...
        and report["parameter_count"] == objects["signature"]["expected_parameter_count"]
        and all_finite
        and any_nonzero
    )
    json_dump(out / "one_batch_preflight.json", report)
    print(json.dumps(report, indent=2))
    print(f"{objects['experiment_spec']['stage_label'].replace('-', '_')}_ONE_BATCH_PREFLIGHT_PASS" if report["pass"] else f"{objects['experiment_spec']['stage_label'].replace('-', '_')}_ONE_BATCH_PREFLIGHT_FAIL")
    return 0 if report["pass"] else 2
...
def smoke_test(args: argparse.Namespace) -> int:
    ...
    report = {
        "stage": f"{objects['experiment_spec']['stage_label']}.3 short training-integrity smoke test",
    ...
    print(json.dumps(report, indent=2))
    print(f"{objects['experiment_spec']['stage_label'].replace('-', '_')}_SMOKE_TEST_PASS" if report["pass"] else f"{objects['experiment_spec']['stage_label'].replace('-', '_')}_SMOKE_TEST_FAIL")
    return 0 if report["pass"] else 2
...
def full_train(args: argparse.Namespace) -> int:
    ...
    set_seed(args.seed)
    if args.seed != 7:
        raise RuntimeError("V4 controlled A1/A2 seed-7 experiments require seed 7.")
    objects = prepare_training_objects(args, include_val=True)
    ...
    summary = {
        ...
        "stage_label": spec["stage_label"],
        "parameter_count": objects["signature"]["parameter_count"],
        "expected_parameter_count": objects["signature"]["expected_parameter_count"],
        "authorization_level": decision.get("authorization_level"),
        ...
        "next_stage": f"{spec['stage_label']}.5 validation-only threshold selection",
    }
    ...
    print(f"\n{spec['stage_label'].replace('-', '_')}_FULL_TRAINING_COMPLETE")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Explicitly authorized V4 A1/A2 Conv1D-GCN memmap training pipeline."
    )
    parser.add_argument(
        "--mode",
        required=True,
    ...
    parser.add_argument(
        ...
        default="a1_frozen",
        help="Strict architecture authorization label.",
    )
    parser.add_argument(
        "--expected-parameter-count",
        type=int,
        default=None,
        help="Must equal the registry count for the chosen designation.",
    )
...
def main() -> int:
    ...
    if args.mode in {"source-audit", "loader-preflight", "one-batch", "smoke", "train"}:
        experiment_spec(args)
