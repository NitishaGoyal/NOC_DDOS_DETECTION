#!/usr/bin/env python3
"""
V4 experiment-controlled validation threshold selection and one-time development-test transfer.
...
EXPERIMENT_REGISTRY = {
    ...
    "a2_capacity_lift": {
        ...
        "test_designation": "development comparison test; reused after A1 analysis",
    },
}
...
def experiment_spec(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected = spec["parameter_count"] if args.expected_parameter_count is None else int(args.expected_parameter_count)
    if expected != int(spec["parameter_count"]):
        raise RuntimeError(
            "Expected-parameter-count authorization mismatch: "
            f"designation={args.experiment_designation} registry={spec['parameter_count']} cli={expected}"
        )
...
def infer(
    ...
    temporal_dim = int(model_args.get("temporal_dim", spec["temporal_dim"]))
    gcn_hidden = int(model_args.get("gcn_hidden", spec["gcn_hidden"]))
    gcn_out = int(model_args.get("gcn_out", spec["gcn_out"]))
    checkpoint_designation = model_args.get("experiment_designation")
    if checkpoint_designation is not None and checkpoint_designation != args.experiment_designation:
        raise RuntimeError(
            f"Checkpoint designation mismatch: checkpoint={checkpoint_designation} cli={args.experiment_designation}"
        )
    actual_dims = (temporal_dim, gcn_hidden, gcn_out)
    expected_dims = (spec["temporal_dim"], spec["gcn_hidden"], spec["gcn_out"])
    if actual_dims != expected_dims:
        raise RuntimeError(
            f"Checkpoint architecture mismatch: checkpoint_dims={actual_dims} authorized_dims={expected_dims}"
        )
    model = TemporalGCN(
        input_features=24,
    ...
    if actual_parameter_count != int(spec["expected_parameter_count"]):
        raise RuntimeError(
            f"Model parameter count drift: actual={actual_parameter_count} expected={spec['expected_parameter_count']}"
        )
...
def validation_mode(args: argparse.Namespace) -> int:
    ...
    pred = infer(
        data_dir, indices, checkpoint, args, args.batch_size, args.num_workers,
    ...
    print(f"{spec['stage_label'].replace('-', '_')}_VALIDATION_THRESHOLD_SELECTION_PASS")
...
def test_mode(args: argparse.Namespace) -> int:
    ...
    spec = experiment_spec(args)
    threshold_designation = thresholds.get("experiment_designation")
    if threshold_designation is not None and threshold_designation != args.experiment_designation:
        raise RuntimeError(
            f"Threshold-package designation mismatch: package={threshold_designation} cli={args.experiment_designation}"
        )
    ...
    metrics = {
        "designation": spec["test_designation"],
        "experiment_designation": spec["experiment_designation"],
        "stage_label": spec["stage_label"],
        "model_name": spec["model_name"],
        "expected_parameter_count": spec["expected_parameter_count"],
    ...
    json_dump(lock, lock_data)
    print(f"{spec['stage_label'].replace('-', '_')}_ONE_TIME_DEVELOPMENT_TEST_TRANSFER_PASS")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-parameter-count", type=int, default=None)
