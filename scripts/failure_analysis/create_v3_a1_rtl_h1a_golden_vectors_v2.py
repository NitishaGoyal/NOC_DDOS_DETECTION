...
def main() -> None:
    ...
    saved_graph_prob = torch.from_numpy(
        graph_prob_reference[selected_local]
    )
    saved_node_prob = torch.from_numpy(
        node_prob_reference[selected_local]
    )

    graph_abs_error = torch.abs(graph_prob - saved_graph_prob)
    node_abs_error = torch.abs(node_prob - saved_node_prob)

    graph_max_abs_error = float(graph_abs_error.max().item())
    node_max_abs_error = float(node_abs_error.max().item())
    ...
    saved_graph_decision = (
        graph_prob_reference[selected_local] >= graph_threshold
    ).astype(np.uint8)
    ...
    saved_node_decision = (
        node_prob_reference[selected_local] >= node_threshold
    ).astype(np.uint8)

    cpu_top1 = np.argmax(node_prob.numpy(), axis=1)
    saved_top1 = np.argmax(
        node_prob_reference[selected_local],
        axis=1,
    )
    ...
    forward_checks = {
        ...
        "graph_prob_matches_saved_cuda_with_tolerance": torch.allclose(
            graph_prob,
            saved_graph_prob,
            atol=probability_atol,
            rtol=probability_rtol,
        ),
        "node_prob_matches_saved_cuda_with_tolerance": torch.allclose(
            node_prob,
            saved_node_prob,
            atol=probability_atol,
            rtol=probability_rtol,
...
Layer-by-layer golden tensors are generated using CPU FP32 inference. The
previously exported validation probabilities were produced using CUDA FP32.
Small CPU/CUDA arithmetic differences are allowed up to absolute tolerance
`1e-4` and relative tolerance `1e-5`, but graph decisions, all 16 node
decisions, and Top-1 router indices must match exactly.
...
"""
    ...
    release = {
        ...
        "reference_policy": {
            "canonical_golden_device": "CPU_FP32",
            "saved_validation_prediction_device": "CUDA_FP32",
            "probability_atol": probability_atol,
            "probability_rtol": probability_rtol,
            "graph_max_abs_error_cpu_vs_saved_cuda": graph_max_abs_error,
            "node_max_abs_error_cpu_vs_saved_cuda": node_max_abs_error,
            "required_discrete_agreement": [
    ...
    print("canonical_golden_device=CPU_FP32")
    print("saved_validation_prediction_device=CUDA_FP32")
    print(f"probability_atol={probability_atol}")
    print(f"probability_rtol={probability_rtol}")
    print(
        "graph_max_abs_error_cpu_vs_saved_cuda="
        f"{graph_max_abs_error:.9e}"
    )
    print(
        "node_max_abs_error_cpu_vs_saved_cuda="
        f"{node_max_abs_error:.9e}"
    )
    print(
        "saved_probability_tolerance_match="
        f"{forward_checks['graph_prob_matches_saved_cuda_with_tolerance'] and forward_checks['node_prob_matches_saved_cuda_with_tolerance']}"
    )
    print(
