...
- the actual model-output CLI option is detected rather than assumed;
...
def main() -> None:
    ...
    detected_model_output_options = [
        option
        for option in MODEL_OUTPUT_OPTION_CANDIDATES
        if option in help_text
    ]
    if len(detected_model_output_options) != 1:
        raise SystemExit(
            "STOP: expected exactly one supported model-output CLI option; "
            f"found={detected_model_output_options}"
        )
    model_output_option = detected_model_output_options[0]
    ...
    all_checks = {
        ...
        **{f"cli_{key}": value for key, value in cli_option_checks.items()},
        "exactly_one_model_output_cli_option_detected": (
            len(detected_model_output_options) == 1
        ),
    ...
    training_plan = {
        ...
        "model_output_path": str(c1_model_dir),
        **{
            key: saved_args[key]
            for key in FROZEN_ARGUMENT_KEYS
        },
    ...
    report = {
        ...
        "cli": {
            ...
            "detected_output_option_candidates": detected_model_output_options,
        },
        "paths": {
