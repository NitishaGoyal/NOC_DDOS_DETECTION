...
def decode_optional_numeric(
    values: np.ndarray,
    *,
    missing_value: float = -1.0,
) -> np.ndarray:
    """Decode numeric metadata while preserving blank normal-run entries.

    The V3 strength array stores normal-run strength as an empty string.
    Blank, None, NaN-like, and non-applicable values are mapped to -1.0.
    Numeric attack strengths are converted to float unchanged.
    """
    decoded = []
    missing_tokens = {
        "",
    ...
    for value in np.asarray(values).reshape(-1):
        ...
        if value is None:
            decoded.append(float(missing_value))
            continue
        ...
        try:
            ...
            raise ValueError(
                f"Unexpected non-numeric metadata value: {value!r}"
            )
...
def infer_validation(
    *,
    ...
    return {
        ...
        "adjacency_nonzero_entries": int(
            torch.count_nonzero(adjacency).item()
        ),
        "adjacency_weight_sum": float(adjacency.sum().item()),
...
def main() -> None:
    ...
    strength = decode_optional_numeric(
        np.asarray(strength_all[validation_idx]),
        missing_value=-1.0,
    )
    ...
    alignment_checks = {
        ...
        "strength_metadata_rows_align": len(strength) == len(validation_idx),
        "strength_metadata_all_finite": bool(np.isfinite(strength).all()),
        "normal_strength_metadata_uses_minus_one": bool(
            np.all(strength[y_graph == 0] == -1.0)
        ),
        "attack_strength_metadata_is_nonnegative": bool(
            np.all(strength[y_graph == 1] >= 0.0)
        ),
        "a1_graph_rows_align": (
        ...
        "a1_adjacency_has_64_nonzero_entries": (
            a1_inference["adjacency_nonzero_entries"] == 64
        ),
        "a1_adjacency_weight_sum_is_positive": (
            a1_inference["adjacency_weight_sum"] > 0.0
        ),
        "a1_adjacency_weight_sum_is_not_edge_count": (
            not math.isclose(
                a1_inference["adjacency_weight_sum"],
                64.0,
                abs_tol=1e-9,
            )
        ),
        "g1_adjacency_has_48_nonzero_entries": (
            g1_inference["adjacency_nonzero_entries"] == 48
        ),
        "g1_adjacency_weight_sum_is_48": math.isclose(
            g1_inference["adjacency_weight_sum"],
            48.0,
            abs_tol=1e-9,
        ),
    ...
    report = {
        ...
        "a1": {
            ...
            "adjacency_nonzero_entries": (
                a1_inference["adjacency_nonzero_entries"]
            ),
            "adjacency_weight_sum": a1_inference["adjacency_weight_sum"],
        },
        "g1": {
            ...
            "adjacency_nonzero_entries": (
                g1_inference["adjacency_nonzero_entries"]
            ),
            "adjacency_weight_sum": g1_inference["adjacency_weight_sum"],
        },
        "gate": {
            "deltas": deltas,
            "primary": primary,
    ...
    print(
        "normal_strength_missing_rows="
        f"{int(np.sum((y_graph == 0) & (strength == -1.0)))}"
    )
    print(
        "attack_strength_numeric_rows="
        f"{int(np.sum((y_graph == 1) & (strength >= 0.0)))}"
    )
    print(
        "a1_adjacency_nonzero_entries="
        f"{a1_inference['adjacency_nonzero_entries']}"
    )
    print(
        "a1_adjacency_weight_sum="
        f"{a1_inference['adjacency_weight_sum']:.9f}"
    )
    print(
        "g1_adjacency_nonzero_entries="
        f"{g1_inference['adjacency_nonzero_entries']}"
    )
    print(
        "g1_adjacency_weight_sum="
        f"{g1_inference['adjacency_weight_sum']:.9f}"
    )
